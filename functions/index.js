...
exports.pushOnNotificationCreated = onDocumentCreated(
  ...
  async (event) => {
    ...
    const message = {
      tokens,
      notification: {title: String(data.title || 'TBT'), body: String(data.body || 'Yeni bir bildirimin var.')},
      data: {
        type,
        sourceId,
        eventId: ['event_join','social_event_join','event_cancelled','social_event_cancelled','community_event','event_memory','campus_digest'].includes(type) ? sourceId : '',
