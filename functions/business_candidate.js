...
exports.createBusinessCandidate = onCall({region:'europe-west1'}, async request=>{
  ...
  const payload={venueKey,venueId,category,venueName,address,city,source:'user_submission',listingStatus:'candidate',verified:false,pendingListing:true,createdBy:uid,createdAt:FieldValue.serverTimestamp(),updatedAt:FieldValue.serverTimestamp()};
  await Promise.all([
    subRef.set({...payload,status:'candidate'}),
    db.collection('business_venues').doc(venueKey).set(payload,{merge:true}),
  ]);
  return {venueId,venueKey,venueName,category};
});
