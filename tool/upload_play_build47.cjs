...
(async()=>{
 ...
 if(!info.includes('applicationId=com.tbt.social')||!info.includes('versionCode=47')||!info.includes('versionName=1.0.19')||!info.includes(sha))throw Error('Artifact identity mismatch');
 ...
 try {
   ...
   if((tracks.tracks||[]).some(t=>(t.releases||[]).some(r=>(r.versionCodes||[]).some(v=>Number(v)>47))))throw Error('A newer Play release exists; refusing to replace it');
   const bundles=await req(url+'/bundles');
   if(!(bundles.bundles||[]).some(b=>Number(b.versionCode)===47)){
     ...
     if(!upload.ok||Number(bundle.versionCode)!==47)throw Error('Bundle upload failed HTTP '+upload.status);
     if(bundle.sha256&&bundle.sha256.toLowerCase()!==sha)throw Error('Uploaded bundle checksum mismatch');
     console.log('Uploaded and verified versionCode 47');
   } else {
     const existing=(bundles.bundles||[]).find(b=>Number(b.versionCode)===47);
     if(!existing.sha256||existing.sha256.toLowerCase()!==sha)throw Error('Existing versionCode 47 has a different bundle; refusing reuse');
     console.log('VersionCode 47 already uploaded; checksum verified');
   }
   await req(url+'/tracks/alpha','PUT',{track:'alpha',releases:[{name:'47 (1.0.19)',versionCodes:['47'],status:'completed',releaseNotes:[{language:'tr-TR',text:'81 ilde kafe, lezzet, otel ve gezilecek yerler için ortak mekan kataloğu eklendi. Kafe kapsamı genişletildi; şehir bazında listeleme ve daha fazla sonuç yükleme iyileştirildi. Rota durakları ve harita seçimi geliştirildi.'}]}]});
   ...
   try {
     ...
     if(!(track.releases||[]).some(r=>(r.versionCodes||[]).includes('47')))throw Error('Committed alpha release could not be read back');
     console.log('PLAY_ALPHA_READBACK '+JSON.stringify(track));
   } finally {await fetch(checkUrl,{method:'DELETE',headers});}
   console.log('PLAY_RELEASE_COMMITTED alpha versionCode=47 review='+review);
   fs.appendFileSync(process.env.GITHUB_STEP_SUMMARY,'Google Play Alpha: 1.0.19 (47) uploaded and committed. Review: '+review+'\nSHA256: '+sha+'\n');
