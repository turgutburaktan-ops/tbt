...
(async()=>{
 ...
 const builds=(await api('/builds?filter[app]='+appId+'&sort=-uploadedDate&limit=30&include=preReleaseVersion')).data;
 console.log(JSON.stringify({versions:versions.map(v=>({id:v.id,version:v.attributes.versionString,state:v.attributes.appStoreState})),builds:builds.map(b=>({id:b.id,number:b.attributes.version,state:b.attributes.processingState}))},null,2));
