...
async function main(){
 const data=JSON.parse(await fs.readFile(path.join(__dirname,'editorial/first-videos.json'),'utf8'));
 check(data.batch==='first-videos-20260908'&&data.posts.length===3,'Unexpected batch');
 check(new Set(data.posts.map(p=>p.id)).size===3,'Duplicate IDs');
 const dir=await fs.mkdtemp('/tmp/tbt-videos-');
 const prepared=[];
 for(const p of data.posts){
  check(/^tbt-rehber-video-0[1-3]$/.test(p.id)&&/^[a-z]+$/.test(p.key),'Unexpected ID');
  check(new URL(p.src).hostname==='videos.pexels.com'&&p.author&&p.licenseUrl&&p.caption,'Unreviewed source');
