...
(async()=>{
 ...
 if(!info.includes('applicationId=com.tbt.social')||!info.includes('versionCode=35')||!info.includes('versionName=1.0.18')||!info.includes(sha))throw Error('Artifact identity mismatch');
 ...
 try {
   ...
   if((tracks.tracks||[]).some(t=>(t.releases||[]).some(r=>(r.versionCodes||[]).some(v=>Number(v)>35))))throw Error('A newer Play release exists; refusing to replace it');
   const bundles=await req(url+'/bundles');
   if(!(bundles.bundles||[]).some(b=>Number(b.versionCode)===35)){
     ...
     if(!upload.ok||Number(bundle.versionCode)!==35)throw Error('Bundle upload failed HTTP '+upload.status);
     if(bundle.sha256&&bundle.sha256.toLowerCase()!==sha)throw Error('Uploaded bundle checksum mismatch');
     console.log('Uploaded and verified versionCode 35');
   } else console.log('VersionCode 35 already uploaded; reusing it');
   await req(url+'/tracks/alpha','PUT',{track:'alpha',releases:[{name:'35 (1.0.18)',versionCodes:['35'],status:'completed',releaseNotes:[{language:'tr-TR',text:'Uygulama genelinde koyu arayüz ve kontroller yenilendi. İşletmeler için detaylı kampanya ve kupon oluşturma, tarih ve kontenjan belirleme, kampanya istatistikleri, QR paylaşımı ve kupon doğrulama eklendi.'}]}]});
   ...
   committed=true;
   console.log('PLAY_RELEASE_COMMITTED alpha versionCode=35 review='+review);
   fs.appendFileSync(process.env.GITHUB_STEP_SUMMARY,'Google Play Alpha: 1.0.18 (35) uploaded and committed. Review: '+review+'\nSHA256: '+sha+'\n');
