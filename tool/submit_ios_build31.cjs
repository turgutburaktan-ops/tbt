...
(async()=>{
 ...
 for(let attempt=0;attempt<40;attempt++){
  const list=await api('/builds?filter[app]='+appId+'&filter[version]=31&include=preReleaseVersion&limit=20');
  ...
  console.log('Waiting for build 31 to finish processing, attempt '+(attempt+1));
  await sleep(30000);
 }
 if(!build||build.attributes.processingState!=='VALID'||build.attributes.expired)throw Error('Build 31 is not ready; current review left unchanged');
 console.log('Verified valid iOS 1.0.18 build 31: '+build.id);
 ...
 if(attached?.id===build.id&&['WAITING_FOR_REVIEW','IN_REVIEW','PENDING_DEVELOPER_RELEASE','READY_FOR_SALE'].includes(version.attributes.appStoreState)){
  console.log('IOS_UPDATE_ALREADY_SUBMITTED build=31 state='+version.attributes.appStoreState);return;
 }
 ...
 if(['WAITING_FOR_REVIEW','IN_REVIEW'].includes(version.attributes.appStoreState)){
  ...
  if(attached?.attributes?.version!=='30')throw Error('Pending review is not build 30; leaving it unchanged');
  await api('/reviewSubmissions/'+active[0].id,'PATCH',{data:{type:'reviewSubmissions',id:active[0].id,attributes:{canceled:true}}});
  console.log('Withdrew previous build 30 for replacement with verified build 31');
 ...
 for(const l of localizations){
  const whatsNew=l.attributes.locale.startsWith('tr')
   ? 'Yeni rota ekranı, araç/yürüyüş/bisiklet seçimi, katılımcılar, sohbet, oylama ve özel gezi albümü eklendi. Herkese açık tarihli rotalar etkinliklerde görünür. Kamera paylaşımı ve galeriden seçim kolaylaştırıldı. Çekim noktası, konum ve açıklama isteğe bağlıdır. iOS kamera yönü düzeltmesi korundu.'
   : 'New route planning with driving, walking and cycling, participants, chat, polls and private trip albums. Public scheduled routes appear in events. Clearer camera sharing and gallery selection. Place name, location and caption are optional. Preserved the iOS camera orientation fix.';
 ...
 console.log('IOS_UPDATE_SUBMITTED version=1.0.18 build=31 state='+final.attributes.state+' review='+review.id);
 if(process.env.GITHUB_STEP_SUMMARY)fs.appendFileSync(process.env.GITHUB_STEP_SUMMARY,'App Store: TBT 1.0.18 (31) submitted. State: '+final.attributes.state+'\n');
