...
(async()=>{
 ...
 if(!info.includes('applicationId=com.tbt.social')||!info.includes('versionCode=30')||!info.includes('versionName=1.0.18')||!info.includes(sha))throw Error('Artifact identity mismatch');
 ...
 try {
   ...
   if((tracks.tracks||[]).some(t=>(t.releases||[]).some(r=>(r.versionCodes||[]).some(v=>Number(v)>30))))throw Error('A newer Play release exists; refusing to replace it');
   const bundles=await req(url+'/bundles');
   if(!(bundles.bundles||[]).some(b=>Number(b.versionCode)===30)){
     ...
     if(!upload.ok||Number(bundle.versionCode)!==30)throw Error('Bundle upload failed HTTP '+upload.status);
     if(bundle.sha256&&bundle.sha256.toLowerCase()!==sha)throw Error('Uploaded bundle checksum mismatch');
     console.log('Uploaded and verified versionCode 30');
   } else console.log('VersionCode 30 already uploaded; reusing it');
   await req(url+'/tracks/alpha','PUT',{track:'alpha',releases:[{name:'30 (1.0.18)',versionCodes:['30'],status:'completed',releaseNotes:[{language:'tr-TR',text:'Keşfet ve Reels geri getirildi. Gezilecek yerlerde şehir seçimi ve sıralama düzeltildi. Seçilen yerler Akıllı Rota sonucunda düzenlenebiliyor. Profil fotoğrafı, takip bağlantıları ve profil sekmeleri düzeltildi. Hata mesajları Türkçeleştirildi.'}]}]});
   ...
   committed=true;
   console.log('PLAY_RELEASE_COMMITTED alpha versionCode=30 review='+review);
   fs.appendFileSync(process.env.GITHUB_STEP_SUMMARY,'Google Play Alpha: 1.0.18 (30) uploaded and committed. Review: '+review+'\nSHA256: '+sha+'\n');
