...
(async()=>{
 ...
 for(let attempt=0;attempt<60;attempt++){
  const list=await api('/builds?filter[app]='+appId+'&filter[version]=47&include=preReleaseVersion&limit=20');
  build=list.data.find(b=>list.included?.find(v=>v.id===b.relationships?.preReleaseVersion?.data?.id)?.attributes?.version==='1.0.19');
  if(build?.attributes.processingState==='VALID')break;
  if(build&&['FAILED','INVALID'].includes(build.attributes.processingState))throw Error('Build 47 processing failed');
  console.log('Waiting for iOS 1.0.19 build 47 processing: '+(attempt+1));
  await sleep(30000);
 }
 if(!build||build.attributes.processingState!=='VALID'||build.attributes.expired)throw Error('Build 47 not ready; existing release left unchanged');
 ...
 const newer=v=>v.attributes.versionString.split('.').map(Number).reduce((n,x)=>n*1000+x,0)>1000019;
 if(versions.some(newer))throw Error('A newer iOS version exists; refusing an older update');
 let version=versions.find(v=>v.attributes.versionString==='1.0.19');
 if(!version){
  const editable=versions.filter(v=>!['READY_FOR_SALE','REMOVED_FROM_SALE','DEVELOPER_REMOVED_FROM_SALE','REPLACED_WITH_NEW_VERSION'].includes(v.attributes.appStoreState));
  if(editable.length)throw Error('Another iOS version is active; refusing to replace it');
  version=(await api('/appStoreVersions','POST',{data:{type:'appStoreVersions',attributes:{platform:'IOS',versionString:'1.0.19',releaseType:'AFTER_APPROVAL',copyright:'2026 TBT'},relationships:{app:{data:{type:'apps',id:appId}}}}})).data;
 }
 ...
 if(attached?.id===build.id&&['WAITING_FOR_REVIEW','IN_REVIEW','PENDING_DEVELOPER_RELEASE','READY_FOR_SALE'].includes(version.attributes.appStoreState)){
  console.log('IOS_UPDATE_ALREADY_SUBMITTED version=1.0.19 build=47 state='+version.attributes.appStoreState);return;
 }
 ...
 for(const l of localizations){
  const whatsNew=l.attributes.locale.startsWith('tr')
   ? '81 ilde kafe, lezzet, otel ve gezilecek yerler için ortak mekan kataloğu eklendi. Kafe kapsamı genişletildi; şehir bazında listeleme ve daha fazla sonuç yükleme iyileştirildi. Rota durakları ve harita seçimi geliştirildi.'
   : 'Added a shared place catalog for cafes, dining, hotels and attractions across all 81 provinces of Türkiye. Expanded cafe coverage and improved city lists, loading more results, route stops and map selection.';
 ...
 console.log('IOS_UPDATE_SUBMITTED version=1.0.19 build=47 state='+final.attributes.state+' review='+review.id+' versionId='+versionId);
 if(process.env.GITHUB_STEP_SUMMARY)fs.appendFileSync(process.env.GITHUB_STEP_SUMMARY,'TBT iOS 1.0.19 (47) submitted: '+final.attributes.state+'\n');
