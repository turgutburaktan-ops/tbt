...
const batch = 'autumn-city-20260909';
...
async function main() {
  ...
  const data = JSON.parse(await fs.readFile(path.join(__dirname, 'editorial/autumn-city-20260909.json'), 'utf8'));
  check(data.batch === batch && data.posts.length === 3 && data.media.length === 3, 'Unexpected batch');
  check(new Set(data.posts.map(p=>p.id)).size === 3, 'Duplicate IDs');
  const buffers = [];
  for (const [i,m] of data.media.entries()) {
    ...
    check(m.author && m.license && /^https:\/\/creativecommons.org\/licenses\/by(-sa)?\/[234]\.0\/$/.test(m.licenseUrl), 'Missing free license');
    check(/^[a-f0-9]{40}$/.test(m.sha1 || ''), 'Missing Commons checksum');
    ...
    check(crypto.createHash('sha1').update(bytes).digest('hex') === m.sha1, 'Commons source version changed');
  ...
  data.posts.forEach((p,i)=>check(p.id === 'tbt-rehber-autumn-city-20260909-0'+(i+1) && p.photo === i && p.status === 'draft' && p.caption && p.city, 'Invalid post'));
  if (prepare) { console.log('Three licensed autumn city photos prepared; no Firebase writes.'); return; }
