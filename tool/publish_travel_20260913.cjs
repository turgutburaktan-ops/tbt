...
async function main(){
 ...
 for(const p of data.posts){
  ...
  execFileSync('ffmpeg',['-v','error','-i',original,'-map','0:v:0','-map','0:a:0','-c:a','aac','-b:a','192k','-c:v','libx264','-preset','fast','-crf','20','-maxrate','10M','-bufsize','20M','-pix_fmt','yuv420p','-movflags','+faststart',video]);
 ...
 console.log(JSON.stringify({newPosts:before.slice(2).filter(s=>!s.exists).length,published:saved.length,originalAudio:true,resolution:'1920x1080',exploreVisible:visible(explore),reelsVisible:visible(reels),ids:refs.map(r=>r.id)}));
