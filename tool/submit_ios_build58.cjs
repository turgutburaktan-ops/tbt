...
(async()=>{
 ...
 for(let attempt=0;attempt<60;attempt++){
  const list=await api('/builds?filter[app]='+appId+'&filter[version]=58&include=preReleaseVersion&limit=20');
  build=list.data.find(b=>list.included?.find(v=>v.id===b.relationships?.preReleaseVersion?.data?.id)?.attributes?.version==='1.0.30');
  if(build?.attributes.processingState==='VALID')break;
  if(build&&['FAILED','INVALID'].includes(build.attributes.processingState))throw Error('Build 58 processing failed');
  console.log('Waiting for iOS 1.0.30 build 58 processing: '+(attempt+1));
  await sleep(30000);
 }
 if(!build||build.attributes.processingState!=='VALID'||build.attributes.expired)throw Error('Build 58 not ready; existing release left unchanged');
 ...
 const newer=v=>v.attributes.versionString.split('.').map(Number).reduce((n,x)=>n*1000+x,0)>1000030;
 if(versions.some(newer))throw Error('A newer iOS version exists; refusing an older update');
 let version=versions.find(v=>v.attributes.versionString==='1.0.30');
 if(!version){
  ...
  version=(await api('/appStoreVersions','POST',{data:{type:'appStoreVersions',attributes:{platform:'IOS',versionString:'1.0.30',releaseType:'AFTER_APPROVAL',copyright:'2026 TBT'},relationships:{app:{data:{type:'apps',id:appId}}}}})).data;
 }
 ...
 if(attached?.id===build.id&&['WAITING_FOR_REVIEW','IN_REVIEW','PENDING_DEVELOPER_RELEASE','READY_FOR_SALE','READY_FOR_DISTRIBUTION'].includes(version.attributes.appStoreState)){
  console.log('IOS_UPDATE_ALREADY_SUBMITTED version=1.0.30 build=58 state='+version.attributes.appStoreState);return;
 }
 ...
 for(const l of localizations){
  const whatsNew=l.attributes.locale.startsWith('tr')
   ? "Video ve hikâye oynatma kararlılığı iyileştirildi. Harita ve görsellerin bellek kullanımı azaltıldı. Sohbet ve rota albümündeki video yüklemeleri iyileştirildi. Yorum silme ve şikâyet seçenekleri eklendi; gönderi sahibi kendi gönderisindeki yorumları silebilir. Çeşitli hata düzeltmeleri yapıldı."
   : "Improved video and Story playback stability, map and image memory usage, and video uploads in chats and route albums. Added comment deletion and reporting, including deletion by the post owner. Includes additional bug fixes.";
 ...
 console.log('IOS_UPDATE_SUBMITTED version=1.0.30 build=58 state='+final.attributes.state+' review='+review.id+' versionId='+versionId);
 if(process.env.GITHUB_STEP_SUMMARY)fs.appendFileSync(process.env.GITHUB_STEP_SUMMARY,'TBT iOS 1.0.30 (58) submitted: '+final.attributes.state+'\n');
