// Add only the Photo moderation/share indexes from the checked-in configuration. Never delete existing indexes.
...
const desired=require('../firestore.indexes.json').indexes.filter(i=>i.collectionGroup==='photo_moderation');
async function api(path,method='GET',body){
 ...
 const value=await response.json();if(!response.ok)throw Error(`Photo moderation index request failed (${response.status})`);return value;
}
async function ensure(spec){
 ...
 while(Date.now()<deadline){const index=await find();if(index?.state==='READY'){console.log(`${spec.collectionGroup} Photo moderation index READY`);return;}if(index?.state==='NEEDS_REPAIR')throw Error(`${spec.collectionGroup} index needs repair`);await new Promise(r=>setTimeout(r,10000));}
