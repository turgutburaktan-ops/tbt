...
(async()=>{
 ...
 if(!info.includes('applicationId=com.tbt.social')||!info.includes('versionCode=31')||!info.includes('versionName=1.0.18')||!info.includes(sha))throw Error('Artifact identity mismatch');
 ...
 try {
   ...
   if((tracks.tracks||[]).some(t=>(t.releases||[]).some(r=>(r.versionCodes||[]).some(v=>Number(v)>31))))throw Error('A newer Play release exists; refusing to replace it');
   const bundles=await req(url+'/bundles');
   if(!(bundles.bundles||[]).some(b=>Number(b.versionCode)===31)){
     ...
     if(!upload.ok||Number(bundle.versionCode)!==31)throw Error('Bundle upload failed HTTP '+upload.status);
     if(bundle.sha256&&bundle.sha256.toLowerCase()!==sha)throw Error('Uploaded bundle checksum mismatch');
     console.log('Uploaded and verified versionCode 31');
   } else console.log('VersionCode 31 already uploaded; reusing it');
   await req(url+'/tracks/alpha','PUT',{track:'alpha',releases:[{name:'31 (1.0.18)',versionCodes:['31'],status:'completed',releaseNotes:[{language:'tr-TR',text:'Yeni rota ekranı, araç/yürüyüş/bisiklet seçimi, katılımcılar, sohbet, oylama ve özel gezi albümü eklendi. Herkese açık tarihli rotalar etkinliklerde görünür. Kamera paylaşımı ve galeriden seçim kolaylaştırıldı. Çekim noktası, konum ve açıklama isteğe bağlıdır. iOS kamera yönü düzeltmesi korundu.'}]}]});
   ...
   committed=true;
   console.log('PLAY_RELEASE_COMMITTED alpha versionCode=31 review='+review);
   fs.appendFileSync(process.env.GITHUB_STEP_SUMMARY,'Google Play Alpha: 1.0.18 (31) uploaded and committed. Review: '+review+'\nSHA256: '+sha+'\n');
