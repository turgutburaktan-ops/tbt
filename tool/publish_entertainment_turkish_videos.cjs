...
async function main(){
 ...
 for(const p of data.posts){
  ...
  const start=Number(p.clipStartSec||0),duration=Number(p.clipDurationSec||sourceProbe.format.duration);
  ...
  console.log(JSON.stringify({prepared:p.id,durationMs,bytes:bytes.length,clipStart:start,audio:'source'}));
 }
 ...
 await db.runTransaction(async tx=>{const snaps=await tx.getAll(user,nameRef,...refs);check(snaps[0].exists&&snaps[0].data().uid===uid&&snaps[1].exists&&snaps[1].data().uid===uid,'Entertainment publisher unavailable');prepared.forEach((e,i)=>{if(snaps[i+2].exists)return;const p=e.p;tx.create(refs[i],{id:p.id,userId:uid,userName:displayName,userPhotoUrl:snaps[0].data().photoURL||'',userEmail:'',caption:`${p.title}\n\n${p.caption}\n\nVideo: ${p.author} / Wikimedia Commons\n${p.sourcePage}\nVideo lisansı: ${p.license} — ${p.licenseUrl}\nTBT Eğlence için ${p.clipStartSec||0}. saniyeden ${p.clipDurationSec} saniyelik bölüm seçilmiş, kaynak ortam sesi korunarak MP4/H.264 + AAC biçimine dönüştürülmüştür.`,spotName:'',city:'',latitude:null,longitude:null,taggedUserIds:[],taggedUserNames:[],likesCount:0,commentsCount:0,sourceType:'post',businessVenueKey:'',businessVenueName:'',businessOfficial:false,venueKey:'',mediaType:'video',imageUrl:e.thumb.url,storagePath:'',videoUrl:e.video.url,videoStoragePath:e.video.path,thumbnailUrl:e.thumb.url,thumbnailStoragePath:e.thumb.path,durationMs:e.durationMs,visibility:'public',status:'published',isEditorial:true,editorialBatch:data.batch,editorialKind:'EntertainmentVideo',videoAuthor:p.author,videoLicense:p.license,videoSourcePage:p.sourcePage,musicId:'',audioMode:'source',createdAt:now,updatedAt:now});});});
 ...
 console.log(JSON.stringify({publisher:username,published:saved.length,exploreVisible:visible(explore),reelsVisible:visible(reels),audio:'source',ids:refs.map(r=>r.id)}));
