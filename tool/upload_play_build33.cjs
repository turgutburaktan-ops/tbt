...
(async()=>{
 ...
 if(!info.includes('applicationId=com.tbt.social')||!info.includes('versionCode=33')||!info.includes('versionName=1.0.18')||!info.includes(sha))throw Error('Artifact identity mismatch');
 ...
 try {
   ...
   if((tracks.tracks||[]).some(t=>(t.releases||[]).some(r=>(r.versionCodes||[]).some(v=>Number(v)>33))))throw Error('A newer Play release exists; refusing to replace it');
   const bundles=await req(url+'/bundles');
   if(!(bundles.bundles||[]).some(b=>Number(b.versionCode)===33)){
     ...
     if(!upload.ok||Number(bundle.versionCode)!==33)throw Error('Bundle upload failed HTTP '+upload.status);
     if(bundle.sha256&&bundle.sha256.toLowerCase()!==sha)throw Error('Uploaded bundle checksum mismatch');
     console.log('Uploaded and verified versionCode 33');
   } else console.log('VersionCode 33 already uploaded; reusing it');
   await req(url+'/tracks/alpha','PUT',{track:'alpha',releases:[{name:'33 (1.0.18)',versionCodes:['33'],status:'completed',releaseNotes:[{language:'tr-TR',text:'İşletme yönetimi uygulamaya eklendi. Menü, kampanya, etkinlik, rezervasyon ve sipariş yönetimi ile kupon QR okutma bir arada. Rota planlama, fotoğraf gösterimi, yeniden paylaşım kartları ve yönetim ekranları iyileştirildi.'}]}]});
   ...
   committed=true;
   console.log('PLAY_RELEASE_COMMITTED alpha versionCode=33 review='+review);
   fs.appendFileSync(process.env.GITHUB_STEP_SUMMARY,'Google Play Alpha: 1.0.18 (33) uploaded and committed. Review: '+review+'\nSHA256: '+sha+'\n');
