...
async function main(){
 const data=JSON.parse(await fs.readFile(path.join(__dirname,'editorial/relaxing-20260911.json'),'utf8'));
 check(data.batch==='relaxing-20260911'&&data.posts.length===1,'Unexpected batch');
 check(new Set(data.posts.map(p=>p.id)).size===1,'Duplicate IDs');
 const dir=await fs.mkdtemp('/tmp/tbt-videos-');
 const prepared=[];
 for(const p of data.posts){
  check(/^tbt-rehber-relaxing-20260911-01$/.test(p.id)&&/^[a-z]+$/.test(p.key),'Unexpected ID');
  check(new URL(p.src).hostname==='upload.wikimedia.org'&&new URL(p.sourcePage).hostname==='commons.wikimedia.org'&&p.author&&p.licenseUrl==='https://creativecommons.org/licenses/by-sa/4.0/'&&p.caption&&p.city==='İstanbul','Unreviewed source');
