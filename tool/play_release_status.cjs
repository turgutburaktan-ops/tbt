...
(async()=>{
 ...
 try {
   ...
   console.log('PLAY_STATUS '+JSON.stringify({tracks:tracks.tracks,bundles:(bundles.bundles||[]).map(b=>({versionCode:b.versionCode,sha256:b.sha256}))}));
 } finally {await req(url,'DELETE');}
