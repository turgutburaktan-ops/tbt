...
(async()=>{
 ...
 for(let attempt=0;attempt<40;attempt++){
  const list=await api('/builds?filter[app]='+appId+'&filter[version]=35&include=preReleaseVersion&limit=20');
  ...
  console.log('Waiting for build 35 to finish processing, attempt '+(attempt+1));
  await sleep(30000);
 }
 if(!build||build.attributes.processingState!=='VALID'||build.attributes.expired)throw Error('Build 35 is not ready; current review left unchanged');
 console.log('Verified valid iOS 1.0.18 build 35: '+build.id);
 ...
 if(attached?.id===build.id&&['WAITING_FOR_REVIEW','IN_REVIEW','PENDING_DEVELOPER_RELEASE','READY_FOR_SALE'].includes(version.attributes.appStoreState)){
  console.log('IOS_UPDATE_ALREADY_SUBMITTED build=35 state='+version.attributes.appStoreState);return;
 }
 ...
 if(['WAITING_FOR_REVIEW','IN_REVIEW'].includes(version.attributes.appStoreState)){
  ...
  if(attached?.attributes?.version!=='33')throw Error('Pending review is not build 33; leaving it unchanged');
  await api('/reviewSubmissions/'+active[0].id,'PATCH',{data:{type:'reviewSubmissions',id:active[0].id,attributes:{canceled:true}}});
  console.log('Withdrew previous build 33 for replacement with verified build 35');
 ...
 for(const l of localizations){
  const whatsNew=l.attributes.locale.startsWith('tr')
   ? 'Uygulama genelinde koyu arayüz ve kontroller yenilendi. İşletmeler için detaylı kampanya ve kupon oluşturma, tarih ve kontenjan belirleme, kampanya istatistikleri, QR paylaşımı ve kupon doğrulama eklendi.'
   : 'Refreshed dark interface and consistent controls throughout the app. Businesses can create detailed campaigns and coupons, set schedules and capacity, view campaign statistics, share QR codes and validate customer coupons.';
 ...
 console.log('IOS_UPDATE_SUBMITTED version=1.0.18 build=35 state='+final.attributes.state+' review='+review.id);
 if(process.env.GITHUB_STEP_SUMMARY)fs.appendFileSync(process.env.GITHUB_STEP_SUMMARY,'App Store: TBT 1.0.18 (35) submitted. State: '+final.attributes.state+'\n');
