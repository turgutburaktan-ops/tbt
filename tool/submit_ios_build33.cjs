...
(async()=>{
 ...
 for(let attempt=0;attempt<40;attempt++){
  const list=await api('/builds?filter[app]='+appId+'&filter[version]=33&include=preReleaseVersion&limit=20');
  ...
  console.log('Waiting for build 33 to finish processing, attempt '+(attempt+1));
  await sleep(30000);
 }
 if(!build||build.attributes.processingState!=='VALID'||build.attributes.expired)throw Error('Build 33 is not ready; current review left unchanged');
 console.log('Verified valid iOS 1.0.18 build 33: '+build.id);
 ...
 if(attached?.id===build.id&&['WAITING_FOR_REVIEW','IN_REVIEW','PENDING_DEVELOPER_RELEASE','READY_FOR_SALE'].includes(version.attributes.appStoreState)){
  console.log('IOS_UPDATE_ALREADY_SUBMITTED build=33 state='+version.attributes.appStoreState);return;
 }
 ...
 if(['WAITING_FOR_REVIEW','IN_REVIEW'].includes(version.attributes.appStoreState)){
  ...
  console.log('Withdrew previous build 30 for replacement with verified build 33');
 ...
 for(const l of localizations){
  const whatsNew=l.attributes.locale.startsWith('tr')
   ? 'İşletme yönetimi uygulamaya eklendi. Menü, kampanya, etkinlik, rezervasyon ve sipariş yönetimi ile kupon QR okutma bir arada. Rota planlama, fotoğraf gösterimi ve yeniden paylaşım kartları iyileştirildi.'
   : 'Manage your business in the app: menus, campaigns, events, reservations, orders and coupon scanning. Improved route planning, photo display and repost cards.';
 ...
 console.log('IOS_UPDATE_SUBMITTED version=1.0.18 build=33 state='+final.attributes.state+' review='+review.id);
 if(process.env.GITHUB_STEP_SUMMARY)fs.appendFileSync(process.env.GITHUB_STEP_SUMMARY,'App Store: TBT 1.0.18 (33) submitted. State: '+final.attributes.state+'\n');
