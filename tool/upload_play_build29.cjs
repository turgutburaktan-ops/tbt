...
(async()=>{
 const bytes=fs.readFileSync('play-artifact/app-release.aab');
 const info=fs.readFileSync('play-artifact/release-info.txt','utf8');
 const sha=crypto.createHash('sha256').update(bytes).digest('hex');
 if(!info.includes('applicationId=com.tbt.social')||!info.includes('versionCode=29')||!info.includes('versionName=1.0.18')||!info.includes(sha))throw Error('Artifact identity mismatch');
 ...
 try {
   ...
   if((tracks.tracks||[]).some(t=>(t.releases||[]).some(r=>(r.versionCodes||[]).some(v=>Number(v)>29))))throw Error('A newer Play release exists; refusing to replace it');
   const bundles=await req(url+'/bundles');
   if(!(bundles.bundles||[]).some(b=>Number(b.versionCode)===29)){
     ...
     if(!upload.ok||Number(bundle.versionCode)!==29)throw Error('Bundle upload failed HTTP '+upload.status);
     if(bundle.sha256&&bundle.sha256.toLowerCase()!==sha)throw Error('Uploaded bundle checksum mismatch');
     console.log('Uploaded and verified versionCode 29');
   } else console.log('VersionCode 29 already uploaded; reusing it');
   await req(url+'/tracks/alpha','PUT',{track:'alpha',releases:[{name:'29 (1.0.18)',versionCodes:['29'],status:'completed',releaseNotes:[{language:'tr-TR',text:'Arama Kişiler, Yerler ve Mekânlar sekmeleriyle sadeleştirildi. Profil bağlantıları, mesajlaşma ve sohbet silme iyileştirildi. Video ve Reels hikâye paylaşımı ile bildirimden yanıt verme geliştirildi.'}]}]});
   ...
   committed=true;
   console.log('PLAY_RELEASE_COMMITTED alpha versionCode=29 review='+review);
   fs.appendFileSync(process.env.GITHUB_STEP_SUMMARY,'Google Play Alpha: 1.0.18 (29) uploaded and committed. Review: '+review+'\nSHA256: '+sha+'\n');
