...
const source=fs.readFileSync('tool/submit_ios_build30.cjs','utf8');
...
async function scenario({invalid=false,extraItem=false,released=false}={}){
 ...
 const fetch=async(url,options)=>{
  ...
  else if(path.startsWith('/builds?'))body={data:[{id:'new-build',attributes:{version:'30',processingState:invalid?'INVALID':'VALID',usesNonExemptEncryption:false},relationships:{preReleaseVersion:{data:{id:'train'}}}}],included:[{id:'train',attributes:{version:'1.0.18'}}]};
  else if(path==='/appStoreVersions/'+versionId+'/build')body={data:{id:attached,attributes:{version:attached==='old-build'?'29':'30'}}};
