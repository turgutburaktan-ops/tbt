...
(async()=>{
 ...
 try {
   ...
   if((tracks.tracks||[]).some(t=>(t.releases||[]).some(r=>(r.versionCodes||[]).some(v=>Number(v)>57))))throw Error('A newer Play release exists; refusing to replace it');
   const bundles=await req(url+'/bundles');
   if(!(bundles.bundles||[]).some(b=>Number(b.versionCode)===57)){
   ...
   try {
     ...
     if(!(track.releases||[]).some(r=>(r.versionCodes||[]).includes('57')))throw Error('Committed alpha release could not be read back');
