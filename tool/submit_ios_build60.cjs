...
(async()=>{
 ...
 for(let attempt=0;attempt<60;attempt++){
  const list=await api('/builds?filter[app]='+appId+'&filter[version]=60&include=preReleaseVersion&limit=20');
  build=list.data.find(b=>list.included?.find(v=>v.id===b.relationships?.preReleaseVersion?.data?.id)?.attributes?.version==='1.0.32');
  if(build?.attributes.processingState==='VALID')break;
  if(build&&['FAILED','INVALID'].includes(build.attributes.processingState))throw Error('Build 60 processing failed');
  console.log('Waiting for iOS 1.0.32 build 60 processing: '+(attempt+1));
  await sleep(30000);
 }
 if(!build||build.attributes.processingState!=='VALID'||build.attributes.expired)throw Error('Build 60 not ready; existing release left unchanged');
 ...
 const newer=v=>v.attributes.versionString.split('.').map(Number).reduce((n,x)=>n*1000+x,0)>1000032;
 if(versions.some(newer))throw Error('A newer iOS version exists; refusing an older update');
 let version=versions.find(v=>v.attributes.versionString==='1.0.32');
 if(!version){
  ...
  version=(await api('/appStoreVersions','POST',{data:{type:'appStoreVersions',attributes:{platform:'IOS',versionString:'1.0.32',releaseType:'AFTER_APPROVAL',copyright:'2026 TBT'},relationships:{app:{data:{type:'apps',id:appId}}}}})).data;
 }
 ...
 if(attached?.id===build.id&&['WAITING_FOR_REVIEW','IN_REVIEW','PENDING_DEVELOPER_RELEASE','READY_FOR_SALE','READY_FOR_DISTRIBUTION'].includes(version.attributes.appStoreState)){
  console.log('IOS_UPDATE_ALREADY_SUBMITTED version=1.0.32 build=60 state='+version.attributes.appStoreState);return;
 }
 ...
 for(const l of localizations){
  const whatsNew=l.attributes.locale.startsWith('tr')
   ? 'Rota oluştururken harita ve liste arasında geçiş ve seçili ile göre hazır rotalar eklendi. Profil gönderi akışı, fotoğraf yakınlaştırma, mesaj açılışı, bildirim gruplaması ve beğenenler listesi iyileştirildi. Mağaza sürüm kontrolü eklendi ve şifre güvenlik kuralları eşitlendi.'
   : 'Switch between map and list while planning routes and discover ready routes by city. Improved profile post browsing, photo zoom, chat opening, notification grouping and likes lists. Added store update checks and consistent password requirements.';
 ...
 console.log('IOS_UPDATE_SUBMITTED version=1.0.32 build=60 state='+final.attributes.state+' review='+review.id+' versionId='+versionId);
 if(process.env.GITHUB_STEP_SUMMARY)fs.appendFileSync(process.env.GITHUB_STEP_SUMMARY,'TBT iOS 1.0.32 (60) submitted: '+final.attributes.state+'\n');
