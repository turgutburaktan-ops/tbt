...
(async()=>{
 ...
 if(!info.includes('applicationId=com.tbt.social')||!info.includes('versionCode=61')||!info.includes('versionName=1.0.33')||!info.includes(sha))throw Error('Artifact identity mismatch');
 ...
 try {
   ...
   if((tracks.tracks||[]).some(t=>(t.releases||[]).some(r=>(r.versionCodes||[]).some(v=>Number(v)>61))))throw Error('A newer Play release exists; refusing to replace it');
   const bundles=await req(url+'/bundles');
   if(!(bundles.bundles||[]).some(b=>Number(b.versionCode)===61)){
     ...
     if(!upload.ok||Number(bundle.versionCode)!==61)throw Error('Bundle upload failed HTTP '+upload.status);
     if(bundle.sha256&&bundle.sha256.toLowerCase()!==sha)throw Error('Uploaded bundle checksum mismatch');
     console.log('Uploaded and verified versionCode 61');
   } else {
     const existing=(bundles.bundles||[]).find(b=>Number(b.versionCode)===61);
     if(!existing.sha256||existing.sha256.toLowerCase()!==sha)throw Error('Existing versionCode 61 has a different bundle; refusing reuse');
     console.log('VersionCode 61 already uploaded; checksum verified');
   }
   const alpha=(tracks.tracks||[]).find(t=>t.track==='alpha');
   if(!alpha)throw Error('Existing alpha track is missing');
   if((alpha.releases||[]).some(r=>!(r.versionCodes||[]).includes('61')&&r.status!=='completed'))throw Error('Another active alpha release exists');
   const release={name:'61 (1.0.33)',versionCodes:['61'],status:'completed',inAppUpdatePriority:2,releaseNotes:[{language:'tr-TR',text:'Hazır rotalardan etkinlik oluşturma ve rota düzenleme deneyimi geliştirildi. Elazığ ve çevre illerin fotoğraflı rota seçkisi genişletildi. Reklam gizlilik tercihleri ve uygulama kararlılığı iyileştirildi.'}]};
   ...
   try {
     ...
     if(!(track.releases||[]).some(r=>(r.versionCodes||[]).includes('61')&&r.status==='completed'))throw Error('Committed alpha release could not be read back');
     const readback=await req(checkUrl+'/bundles');
     if(!(readback.bundles||[]).some(b=>Number(b.versionCode)===61&&b.sha256?.toLowerCase()===sha))throw Error('Read-back bundle checksum mismatch');
     console.log('PLAY_ALPHA_READBACK '+JSON.stringify(track));
   } finally {await fetch(checkUrl,{method:'DELETE',headers});}
   console.log('PLAY_RELEASE_COMMITTED alpha versionCode=61 review='+review);
   fs.appendFileSync(process.env.GITHUB_STEP_SUMMARY,'Google Play Alpha: 1.0.33 (61) submitted to closed testing. Review: '+review+'\nSHA256: '+sha+'\n');
