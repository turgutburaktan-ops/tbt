...
(async()=>{
 ...
 if(!info.includes('applicationId=com.tbt.social')||!info.includes('versionCode=58')||!info.includes('versionName=1.0.30')||!info.includes(sha))throw Error('Artifact identity mismatch');
 ...
 try {
   ...
   if((tracks.tracks||[]).some(t=>(t.releases||[]).some(r=>(r.versionCodes||[]).some(v=>Number(v)>58))))throw Error('A newer Play release exists; refusing to replace it');
   const bundles=await req(url+'/bundles');
   if(!(bundles.bundles||[]).some(b=>Number(b.versionCode)===58)){
     ...
     if(!upload.ok||Number(bundle.versionCode)!==58)throw Error('Bundle upload failed HTTP '+upload.status);
     if(bundle.sha256&&bundle.sha256.toLowerCase()!==sha)throw Error('Uploaded bundle checksum mismatch');
     console.log('Uploaded and verified versionCode 58');
   } else {
     const existing=(bundles.bundles||[]).find(b=>Number(b.versionCode)===58);
     if(!existing.sha256||existing.sha256.toLowerCase()!==sha)throw Error('Existing versionCode 58 has a different bundle; refusing reuse');
     console.log('VersionCode 58 already uploaded; checksum verified');
   }
   await req(url+'/tracks/alpha','PUT',{track:'alpha',releases:[{name:'58 (1.0.30)',versionCodes:['58'],status:'completed',releaseNotes:[{language:'tr-TR',text:"Video ve hikâye oynatma kararlılığı iyileştirildi. Harita ve görsellerin bellek kullanımı azaltıldı. Sohbet ve rota albümündeki video yüklemeleri iyileştirildi. Yorum silme ve şikâyet seçenekleri eklendi; gönderi sahibi kendi gönderisindeki yorumları silebilir. Çeşitli hata düzeltmeleri yapıldı."}]}]});
   ...
   try {
     ...
     if(!(track.releases||[]).some(r=>(r.versionCodes||[]).includes('58')))throw Error('Committed alpha release could not be read back');
     console.log('PLAY_ALPHA_READBACK '+JSON.stringify(track));
   } finally {await fetch(checkUrl,{method:'DELETE',headers});}
   console.log('PLAY_RELEASE_COMMITTED alpha versionCode=58 review='+review);
   fs.appendFileSync(process.env.GITHUB_STEP_SUMMARY,'Google Play Alpha: 1.0.30 (58) uploaded and committed. Review: '+review+'\nSHA256: '+sha+'\n');
