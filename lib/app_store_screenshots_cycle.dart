...
class _ScreenshotApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    const screens = ['home', 'places', 'detail', 'plan', 'nearby', 'profile'];
    return StreamBuilder<int>(
      stream: Stream<int>.periodic(const Duration(seconds: 10), (index) => index + 1),
      initialData: 0,
      builder: (context, snapshot) {
        final screen = screens[(snapshot.data ?? 0) % screens.length];
        return MaterialApp(
