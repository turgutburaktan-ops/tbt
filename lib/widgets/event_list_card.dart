      ...
      child: Container(padding: const EdgeInsets.all(13), decoration: BoxDecoration(borderRadius: BorderRadius.circular(17), border: Border.all(color: AppColors.border)), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Text(event.locationLabel.isEmpty ? event.city : event.locationLabel, maxLines: 2, overflow: TextOverflow.ellipsis, style: const TextStyle(color: AppColors.textMuted, fontSize: 13)),
          ])),
        ]),
        const SizedBox(height: 10),
        Wrap(spacing: 12, runSpacing: 4, crossAxisAlignment: WrapCrossAlignment.center, children: [
          Text('${event.participantCount}/${event.capacity} katılımcı', style: const TextStyle(color: AppColors.textMuted, fontSize: 12)),
          if (remaining > 0 && remaining <= 2) Text('Son $remaining yer', style: const TextStyle(color: Colors.orangeAccent, fontSize: 12)),
          if (joined)
            TextButton.icon(onPressed: busy ? null : onOpen,
              icon: const Icon(Icons.check_circle_outline, size: 17),
              label: const Text('Detayları Gör'))
          else
            FilledButton(onPressed: busy || event.isFull ? null : onJoin,
              child: busy ? const SizedBox(width:15,height:15,child:CircularProgressIndicator(strokeWidth:2)) : Text(event.isFull ? 'Dolu' : 'Ben de Geliyorum')),
