...
class _BusinessPublicActionsState extends State<BusinessPublicActions>{
  ...
  bool _following=false,_loading=true,_busy=false,_serverReservations=false;
  ...
  Future<void> _load()async{
    ...
    try{
      final results=await Future.wait([
        _functions.httpsCallable('getBusinessFollowStatus').call({'venueKey':widget.venueKey}),
        _functions.httpsCallable('getBusinessPublicFeatures').call({'venueKey':widget.venueKey}),
        _functions.httpsCallable('recordBusinessMetric').call({'venueKey':widget.venueKey,'metric':'profile_view'}),
      ]);
      final follow=Map<String,dynamic>.from(results[0].data as Map);
      final features=Map<String,dynamic>.from(results[1].data as Map);
      if(mounted)setState((){_following=follow['following']==true;_serverReservations=features['reservationsEnabled']==true;});
  ...
  @override Widget build(BuildContext context){if(FirebaseAuth.instance.currentUser==null)return const SizedBox.shrink();final reservationVisible=widget.reservationsEnabled&&_serverReservations;return Row(children:[Expanded(child:OutlinedButton.icon(onPressed:_loading||_busy?null:_follow,icon:_loading?const SizedBox(width:16,height:16,child:CircularProgressIndicator(strokeWidth:2)):Icon(_following?Icons.notifications_active_rounded:Icons.notifications_none_rounded),label:Text(_following?'Takip Ediliyor':'Takip Et'))),if(reservationVisible)...[const SizedBox(width:8),Expanded(child:FilledButton.icon(onPressed:_reservation,icon:const Icon(Icons.event_seat_outlined),label:const Text('Rezervasyon')))] ]);}
}
