...
class ProfileFavoritePlacesSection extends StatelessWidget {
                          ...
                          Expanded(
                            child: Column(
                              ...
                              children: [
                                ...
                                Text(name.isEmpty ? 'Favorini seç' : name, maxLines: 1, overflow: TextOverflow.ellipsis, style: TextStyle(fontWeight: FontWeight.w900, color: name.isEmpty ? Colors.white38 : Colors.white)),
                                if (subtitle.isNotEmpty) ...[
                                  const SizedBox(height: 2),
                                  Text(subtitle, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: Colors.white46, fontSize: 11.5)),
                                ],
                              ],
                            ),
                          ),
  ...
  Future<void> _pick(BuildContext context, _FavoriteType type) async {
    ...
    try {
      if (selected.remove) {
        await FirebaseFirestore.instance.collection('users').doc(userId).update({'favoritePlaces.${type.key}': FieldValue.delete()});
      } else {
...
class _FavoritePlacePickerState extends State<_FavoritePlacePicker> {
  ...
  Future<List<_PlaceChoice>> _load() async {
    if (widget.type.key == 'spot') {
      final snap = await FirebaseFirestore.instance.collection('photo_spots').where('status', isEqualTo: 'published').limit(500).get();
      return snap.docs.map((doc) {
        final d = doc.data();
        return _PlaceChoice(id: doc.id, name: (d['name'] ?? 'Gezilecek yer').toString(), subtitle: (d['city'] ?? d['district'] ?? '').toString());
      }).toList()..sort((a, b) => a.name.compareTo(b.name));
    }
    final snap = await FirebaseFirestore.instance.collection('business_venues').where('category', isEqualTo: widget.type.key).limit(500).get();
    return snap.docs.map((doc) {
      final d = doc.data();
      final city = (d['city'] ?? '').toString().trim();
      final address = (d['address'] ?? '').toString().trim();
      return _PlaceChoice(id: doc.id, name: (d['venueName'] ?? d['name'] ?? 'Mekan').toString(), subtitle: city.isNotEmpty ? city : address);
    }).toList()..sort((a, b) => a.name.compareTo(b.name));
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(title: Text(widget.type.label), actions: [
        TextButton(onPressed: () => Navigator.pop(context, const _PlaceChoice.remove()), child: const Text('Kaldır')),
      ]),
      body: Column(
        children: [
          Padding(
          ...
          Expanded(
            child: FutureBuilder<List<_PlaceChoice>>(
              ...
              builder: (context, snapshot) {
                ...
                if (snapshot.hasError) return const Center(child: Text('Mekanlar yüklenemedi.', style: TextStyle(color: Colors.white54)));
                final q = _search.text.trim().toLowerCase();
                final items = (snapshot.data ?? const <_PlaceChoice>[]).where((item) => q.isEmpty || '${item.name} ${item.subtitle}'.toLowerCase().contains(q)).toList();
...
class _PlaceChoice {
  ...
  final bool remove;
  const _PlaceChoice({required this.id, required this.name, required this.subtitle}) : remove = false;
  const _PlaceChoice.remove() : id = '', name = '', subtitle = '', remove = true;
}
