...
class ProfileReservations extends StatefulWidget {
  final String userId;
  const ProfileReservations({super.key,required this.userId});
  @override
...
class _ProfileReservationsState extends State<ProfileReservations> with WidgetsBindingObserver {
  ...
  Widget build(BuildContext context)=>Card(
    ...
    child:ExpansionTile(title:const Text('Rezervasyonlarım'),leading:const Icon(Icons.event_seat_outlined),
      ...
      children:[
        ...
        ...?_items?.map((d){
          ...
          final status={'pending':'Yanıt bekliyor','accepted':'Onaylandı','rejected':'Reddedildi'}[d['status']]??'Sonuçlandı';
          ...
          return ListTile(title:Text('${d['venueName']} · $status'),subtitle:Text('${at.day}.${at.month}.${at.year} ${at.hour.toString().padLeft(2,'0')}:${at.minute.toString().padLeft(2,'0')} · ${d['partySize']} kişi\n${d['note']??''}${orders.isEmpty?'':'\n$orders\nSipariş toplamı: $total TL'}'));
        }),
      ],
    ),
  );
