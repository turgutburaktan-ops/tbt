                  ...
                  child: Ink(
                    ...
                    child: const Row(
                      ...
                      children: [
                        _PulseDot(),
                        SizedBox(width: 6),
                        Text('Şu An', style: TextStyle(fontSize: 11, fontWeight: FontWeight.w800)),
                        SizedBox(width: 3),
                        Icon(Icons.keyboard_arrow_up_rounded, size: 15, color: Colors.white54),
                      ],
                    ),
                  ),
                ),
...
class _PulseDotState extends State<_PulseDot> with SingleTickerProviderStateMixin {
  ...
  @override void initState() { super.initState(); _controller = AnimationController(vsync: this, duration: const Duration(milliseconds: 1200))..repeat(reverse: true); }
  ...
  @override Widget build(BuildContext context) => FadeTransition(
        ...
        child: const SizedBox(width: 8, height: 8, child: DecoratedBox(decoration: BoxDecoration(shape: BoxShape.circle, color: Color(0xFF45E7F2)))),
      );
}
...
class _NowSheet extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final recentPosts = FirebaseFirestore.instance.collection('posts').orderBy('createdAt', descending: true).limit(60);
    final upcomingEvents = FirebaseFirestore.instance.collection('social_events').orderBy('startsAt').limit(40);
    return SafeArea(
      top: false,
      child: FractionallySizedBox(
        ...
        child: Container(
          ...
          child: Column(children: [
            ...
            Container(width: 38, height: 4, decoration: BoxDecoration(color: Colors.white24, borderRadius: BorderRadius.circular(999))),
            const Padding(
              padding: EdgeInsets.fromLTRB(20, 17, 20, 8),
              child: Row(children: [
                Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Text('Şu An', style: TextStyle(fontSize: 24, fontWeight: FontWeight.w900, letterSpacing: -.5)),
                  SizedBox(height: 3),
                  Text('Çevrende ve TBT’de son hareketler', style: TextStyle(color: Colors.white54, fontSize: 12.5)),
                ])),
                _RadarBadge(),
              ]),
            ),
            Expanded(
              child: StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
                ...
                builder: (context, postsSnapshot) => StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
                  stream: upcomingEvents.snapshots(),
                  builder: (context, eventsSnapshot) {
                    final now = DateTime.now();
                    final cutoff = now.subtract(const Duration(hours: 3));
                    final posts = (postsSnapshot.data?.docs ?? const []).where((doc) {
                      final t = doc.data()['createdAt'];
                      return t is Timestamp && t.toDate().isAfter(cutoff);
                    }).toList();
                    final events = (eventsSnapshot.data?.docs ?? const []).where((doc) {
                      final t = doc.data()['startsAt'];
                      if (t is! Timestamp) return false;
                      final date = t.toDate();
                      return date.isAfter(now.subtract(const Duration(hours: 1))) && date.isBefore(now.add(const Duration(hours: 24)));
                    }).toList();
                    final creators = posts.map((d) => (d.data()['userId'] ?? '').toString()).where((e) => e.isNotEmpty).toSet().length;
                    return ListView(
                      ...
                      children: [
                        ...
                        if (posts.isEmpty) const _Empty('Henüz yeni hareket yok. İlk paylaşımı sen yapabilirsin.')
                        else ...posts.take(4).map((d) => _PostTile(d.data())),
                        const SizedBox(height: 18),
                        const _Section(icon: Icons.event_available_rounded, title: 'Önümüzdeki 24 saat'),
                        const SizedBox(height: 8),
                        if (events.isEmpty) const _Empty('Yaklaşan herkese açık etkinlik görünmüyor.')
                        else ...events.take(4).map((d) => _EventTile(d.data())),
                        const SizedBox(height: 18),
...
class _RadarBadge extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) => Container(
    width: 42, height: 42,
    decoration: BoxDecoration(color: const Color(0x1645E7F2), borderRadius: BorderRadius.circular(14), border: Border.all(color: const Color(0x3345E7F2))),
...
class _StatsCard extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) => Container(
    ...
    decoration: BoxDecoration(color: const Color(0xFF111620), borderRadius: BorderRadius.circular(18), border: Border.all(color: Colors.white.withValues(alpha: .06))),
    child: Row(children: [
...
class _PostTile extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) {
    ...
    return _InfoTile(icon:Icons.photo_camera_back_rounded,title:name,subtitle:spot.isNotEmpty?'$spot konumundan yeni paylaşım':(caption.isNotEmpty?caption:'Yeni paylaşım yaptı'));
  }
}
class _EventTile extends StatelessWidget {
  final Map<String,dynamic> data; const _EventTile(this.data);
  @override Widget build(BuildContext context) {
    final title=(data['title']??data['name']??'Etkinlik').toString();
    final raw=data['startsAt']; final date=raw is Timestamp?raw.toDate().toLocal():null;
    final time=date==null?'':'${date.hour.toString().padLeft(2,'0')}:${date.minute.toString().padLeft(2,'0')}';
    final location=[data['locationLabel'],data['venueName'],data['city']].map((e)=>(e??'').toString().trim()).firstWhere((e)=>e.isNotEmpty,orElse:()=> '');
    return _InfoTile(icon:Icons.event_rounded,title:title,subtitle:[if(time.isNotEmpty)time,if(location.isNotEmpty)location].join(' • '));
  }
}
class _InfoTile extends StatelessWidget {
  final IconData icon; final String title,subtitle; const _InfoTile({required this.icon,required this.title,required this.subtitle});
  @override Widget build(BuildContext context)=>Container(
...
class _FriendActivity extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) {
    ...
    if(uid==null)return const _Empty('Arkadaş hareketini görmek için giriş yap.');
    return StreamBuilder<QuerySnapshot<Map<String,dynamic>>>(
      stream:FirebaseFirestore.instance.collection('users').doc(uid).collection('following').limit(50).snapshots(),
      builder:(context,following){
        final ids=following.data?.docs.map((d)=>d.id).toSet()??<String>{};
        if(ids.isEmpty)return const _Empty('Takip ettiğin kişiler paylaşım yaptığında burada görünecek.');
        return StreamBuilder<QuerySnapshot<Map<String,dynamic>>>(
          stream:FirebaseFirestore.instance.collection('posts').orderBy('createdAt',descending:true).limit(80).snapshots(),
          builder:(context,posts){
            final items=(posts.data?.docs??const[]).where((d)=>ids.contains((d.data()['userId']??'').toString())).take(4).toList();
            if(items.isEmpty)return const _Empty('Takip ettiklerinden yeni hareket yok.');
            return Column(children:items.map((d){final x=d.data();return _InfoTile(icon:Icons.people_rounded,title:(x['userName']??'Takip ettiğin biri').toString(),subtitle:(x['spotName']??'').toString().isNotEmpty?'${x['spotName']} konumundan paylaşım yaptı':'Yeni paylaşım yaptı');}).toList());
          },
        );
      },
    );
