...
class CommunityService {
  ...
  Future<void> toggleFollow(String communityId) async {
    ...
    if (user == null)
      throw Exception('Topluluğu takip etmek için giriş yapmalısın.');
  ...
  Future<String> createCommunity({
    ...
    final user = _auth.currentUser;
    if (user == null)
      throw Exception('Topluluk oluşturmak için giriş yapmalısın.');
    final cleanName = name.trim();
    if (cleanName.length < 3)
      throw Exception('Topluluk adı en az 3 karakter olmalı.');
    ...
    await ref.collection('followers').doc(user.uid).set({
      'userId': user.uid,
      'createdAt': FieldValue.serverTimestamp(),
    });
    return ref.id;
