        ...
        builder: (context, events) {
          ...
          return ListView(
            ...
            children: [
              ...
              Text('${_dayName(now.weekday)} • Bugünün öne çıkanları', style: const TextStyle(color: Colors.white54, fontSize: 12.5)),
              const SizedBox(height: 16),
              Row(children: [
                Expanded(child: _MetricCard(icon: Icons.photo_library_rounded, value: '${recent.length}', label: 'son 12 saatte')),
              ...
              if (upcoming.isEmpty)
                ...
                ...upcoming.take(6).map((d) {
                  final x = d.data(), t = x['startsAt'];
                  final date = t is Timestamp ? t.toDate().toLocal() : null;
                  final place = [x['locationLabel'], x['venueName'], x['city']]
                  ...
                  return _SimpleTile(
                    ...
                    subtitle: [
                      if (date != null) '${date.hour.toString().padLeft(2, '0')}:${date.minute.toString().padLeft(2, '0')}',
...
class _LiveSocialMapTabState extends State<_LiveSocialMapTab> {
  ...
  Future<void> _focusUser() async {
    ...
    if (p != null) {
      ...
      try {
        await _map?.animateCamera(CameraUpdate.newCameraPosition(CameraPosition(target: _user!, zoom: 12.6)));
      } catch (_) {}
    }
  ...
  Future<void> _fitSignals(Set<Marker> markers) async {
    if (_fittedOnce || _user != null || markers.isEmpty || _map == null) return;
    _fittedOnce = true;
    final points = markers.map((m) => m.position).toList();
    try {
      if (points.length == 1) {
        await _map!.animateCamera(CameraUpdate.newCameraPosition(CameraPosition(target: points.first, zoom: 11.5)));
        return;
      }
      ...
      builder: (context, eventSnapshot) => StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
        ...
        builder: (context, postSnapshot) {
          ...
          for (final doc in eventSnapshot.data?.docs ?? const []) {
            final d = doc.data();
            final lat = (d['latitude'] as num?)?.toDouble(), lng = (d['longitude'] as num?)?.toDouble(), starts = d['startsAt'];
          ...
          for (final doc in postSnapshot.data?.docs ?? const []) {
            final d = doc.data(), t = d['createdAt'];
            final lat = (d['latitude'] as num?)?.toDouble(), lng = (d['longitude'] as num?)?.toDouble();
            if (lat == null || lng == null || t is! Timestamp || t.toDate().isBefore(recent)) continue;
            markers.add(Marker(
              markerId: MarkerId('post_${doc.id}'),
              position: LatLng(lat, lng),
              icon: BitmapDescriptor.defaultMarkerWithHue(BitmapDescriptor.hueCyan),
              infoWindow: InfoWindow(title: (d['spotName'] ?? 'Yeni paylaşım').toString(), snippet: 'Son 6 saatte hareket'),
            ));
          }
          ...
          return Stack(children: [
            GoogleMap(
              ...
              onMapCreated: (c) {
                _map = c;
                if (_user != null) c.moveCamera(CameraUpdate.newCameraPosition(CameraPosition(target: _user!, zoom: 12.6)));
              },
            ),
            Positioned(
              left: 12,
              ...
              child: Container(
                ...
                decoration: BoxDecoration(color: const Color(0xE6101319), borderRadius: BorderRadius.circular(16), border: Border.all(color: Colors.white10)),
                child: Row(children: [
                  const Icon(Icons.radar_rounded, color: Color(0xFF45E7F2), size: 21),
                  const SizedBox(width: 8),
                  Expanded(child: Text('${markers.length} topluluk sinyali', style: const TextStyle(fontWeight: FontWeight.w900, fontSize: 13))),
                  const Text('Kişi konumu gösterilmez', style: TextStyle(color: Color(0x75FFFFFF), fontSize: 9.5)),
                ]),
              ),
            ),
            Positioned(
...
class _MixedDiscoveryTab extends StatelessWidget {
  ...
  bool _hasVisual(Map<String, dynamic> d) => ['imageUrl', 'storagePath', 'thumbnailUrl', 'thumbnailStoragePath']
  ...
  Widget build(BuildContext context) => StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
    ...
    builder: (context, snapshot) {
      ...
      if (docs.isEmpty) {
        return const _RichEmpty(
          icon: Icons.explore_outlined,
          title: 'Keşif hazırlanıyor',
          text: 'Görsel paylaşımlar geldikçe sana benzer içerikleri burada göstereceğiz.',
        );
      }
      return LayoutBuilder(
        builder: (context, constraints) {
          ...
          return GridView.builder(
            ...
            itemBuilder: (context, i) => _DiscoveryCard(postId: docs[i].id, data: docs[i].data()),
          );
        },
      );
    },
  ...
  Widget build(BuildContext context) {
    ...
    if (uid == null) {
      return _RichEmpty(
        ...
        title: 'Şehir keşfini kaydet',
        text: 'Şehirlerini ve keşfettiğin noktaları biriktirmek için hesabına giriş yap.',
        action: 'TBT’de ara',
    ...
    return StreamBuilder<DocumentSnapshot<Map<String, dynamic>>>(
      ...
      builder: (context, profile) => StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
        ...
        builder: (context, posts) {
          ...
          if (raw is Iterable) configured.addAll(raw.map((e) => e.toString().trim()).where((e) => e.isNotEmpty));
          final cityCounts = <String, int>{}, uniqueSpots = <String>{};
          for (final doc in posts.data?.docs ?? const []) {
            final d = doc.data(), city = (d['city'] ?? '').toString().trim(), spot = (d['spotName'] ?? '').toString().trim();
            if (city.isNotEmpty) {
              configured.add(city);
          ...
          return ListView(
            padding: const EdgeInsets.fromLTRB(16, 18, 16, 30),
            children: [
              const Text('Keşif Haritan', style: TextStyle(fontSize: 26, fontWeight: FontWeight.w900)),
              const SizedBox(height: 4),
              Text('${list.length} şehir • ${uniqueSpots.length} farklı nokta', style: const TextStyle(color: Colors.white54)),
              const SizedBox(height: 18),
              if (list.isEmpty)
                _RichEmpty(
                  icon: Icons.map_outlined,
                  title: 'İlk şehrini keşfet',
                  text: 'Şehir veya konum bilgisi olan paylaşımların burada kişisel keşif haritana dönüşür.',
                  action: 'TBT’de şehir ara',
                  onAction: () => Navigator.pushNamed(context, '/search'),
                  compact: true,
                )
              else
                ...list.map((city) {
                  ...
                  final progress = (count / 10).clamp(.05, 1.0);
                  return Container(
                    ...
                    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Row(children: [
                        ...
                        Text('$count paylaşım', style: const TextStyle(color: Color(0x75FFFFFF), fontSize: 10.5)),
                      ]),
                      const SizedBox(height: 9),
                      LinearProgressIndicator(value: progress, minHeight: 6, borderRadius: BorderRadius.circular(999)),
                    ]),
                  );
                }),
            ],
...
class _RichEmpty extends StatelessWidget {
  final IconData icon;
  final String title;
  final String text;
  final String? action;
  ...
  final bool compact;
  const _RichEmpty({required this.icon, required this.title, required this.text, this.action, this.onAction, this.compact = false});
    ...
    child: Container(
      ...
      child: Column(mainAxisSize: MainAxisSize.min, children: [
        Container(
          width: 56,
          height: 56,
          decoration: BoxDecoration(color: const Color(0xFF45E7F2).withValues(alpha: .08), shape: BoxShape.circle),
          child: Icon(icon, color: const Color(0xFF45E7F2), size: 28),
        ),
        const SizedBox(height: 12),
        Text(title, textAlign: TextAlign.center, style: const TextStyle(fontSize: 18, fontWeight: FontWeight.w900)),
        const SizedBox(height: 6),
        Text(text, textAlign: TextAlign.center, style: const TextStyle(color: Colors.white54, height: 1.4)),
        if (action != null && onAction != null) ...[
          const SizedBox(height: 16),
          FilledButton.icon(onPressed: onAction, icon: const Icon(Icons.search_rounded), label: Text(action!)),
        ],
      ]),
    ),
  );
...
class _MetricCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
    ...
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      Text(label, style: const TextStyle(color: Color(0x75FFFFFF), fontSize: 10.5)),
    ]),
  );
}
...
class _SectionTitle extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => Text(text, style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w900));
}
  ...
  Widget build(BuildContext context) => Container(
    ...
    child: Row(children: [
      ...
      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        if (subtitle.isNotEmpty) Text(subtitle, maxLines: 2, overflow: TextOverflow.ellipsis, style: const TextStyle(color: Color(0x75FFFFFF), fontSize: 11)),
      ])),
    ]),
  );
}