...
class _NotificationsScreenState extends State<NotificationsScreen> {
  ...
  Future<Map<String, dynamic>> _actor(String id) =>
      _actorCache.putIfAbsent(id, () => _loadActor(id));
  ...
  Future<void> _openItem(AppNotificationItem item) async {
    ...
    if (item.type == 'tbt_broadcast') {
      await Navigator.push(
        context,
        MaterialPageRoute(builder: (_) => BroadcastDetailScreen(item: item)),
      );
      return;
    }
    if (item.type.startsWith('business_preparation') ||
        item.type.startsWith('business_reservation')) {
      final owner =
          item.type == 'business_reservation' ||
          item.type == 'business_reservation_owner_action';
      await Navigator.push(
        context,
  ...
  String _timeLabel(DateTime? value) {
    ...
    if (diff.inDays == 1)
      return 'Dün ${local.hour.toString().padLeft(2, '0')}:${local.minute.toString().padLeft(2, '0')}';
  ...
  Widget _iconBubble(String type) {
    ...
    return Container(
      ...
      child: Icon(_iconFor(type), color: accent, size: 23),
    );
  }
  ...
  Widget _leading(AppNotificationItem item) {
    if (item.type == 'tbt_broadcast') {
      return const CircleAvatar(
        radius: 25,
        backgroundColor: Color(0xFF29444D),
        child: Text(
          'TBT',
    ...
    appBar: AppBar(
      ...
      actions: [
        PopupMenuButton<String>(
          ...
          onSelected: (v) {
            if (v == 'read_all') AppNotificationService.instance.markAllRead();
          },
          itemBuilder: (_) => const [
            PopupMenuItem(
              value: 'read_all',
                  ...
                  : ListView.builder(
                      ...
                      itemBuilder: (context, index) {
                        final item = items[index],
                            group = _groupLabel(item.createdAt),
                            previous = index == 0
                                ? null
