  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: CustomScrollView(slivers: [
        SliverAppBar(
          expandedHeight: 320, pinned: true, backgroundColor: const Color(0xFF090A0C), foregroundColor: Colors.white,
          actions: [
            IconButton(tooltip: 'Paylaş', onPressed: () => InviteLinkService.instance.shareSpot(spotId: spot.id, spotName: spot.name, city: spot.city, latitude: spot.latitude, longitude: spot.longitude), icon: const Icon(Icons.ios_share_rounded)),
            ValueListenableBuilder<List<PhotoSpot>>(valueListenable: FavoritesService.savedSpots, builder: (context, savedSpots, _) {
          ...
          flexibleSpace: FlexibleSpaceBar(background: GestureDetector(onTap: () => _openPhoto(context), child: Stack(fit: StackFit.expand, children: [
        ...
        SliverPadding(padding: const EdgeInsets.fromLTRB(16, 18, 16, 40), sliver: SliverList.list(children: [
          ...
          Row(children: [
            Expanded(child: FilledButton.icon(onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const CameraScreen())), icon: const Icon(Icons.camera_alt_outlined), label: const Text('Burada Çek'))),
