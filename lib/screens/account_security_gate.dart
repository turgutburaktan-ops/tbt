...
class AccountSecurityGate extends StatefulWidget {
  ...
  const AccountSecurityGate({super.key, required this.child, required this.profile});
...
class _AccountSecurityGateState extends State<AccountSecurityGate> {
  ...
  Future<void> _evaluate() async {
    if (!_phoneVerified) {
      if (mounted) setState(() { _checking = false; _unlocked = false; });
      return;
    }
    ...
    if (available && enabled) await _unlockWithBiometric();
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (!_phoneVerified) return const PhoneSecuritySetupScreen();
          ...
          child: Center(
            child: Padding(
              ...
              child: Column(mainAxisSize: MainAxisSize.min, children: [
                ...
                const Text('Hesabını açmak için parmak izi veya cihaz biyometrisini kullan.', textAlign: TextAlign.center, style: TextStyle(color: Colors.white60, height: 1.4)),
                const SizedBox(height: 22),
                FilledButton.icon(onPressed: _unlockWithBiometric, icon: const Icon(Icons.fingerprint_rounded), label: const Text('Parmak iziyle aç')),
                TextButton(onPressed: () => FirebaseAuth.instance.signOut(), child: const Text('Başka hesapla giriş yap')),
              ]),
            ),
          ),
        ),
              ...
              return Material(
                ...
                child: InkWell(
                  ...
                  child: const Padding(
                    ...
                    child: Row(mainAxisSize: MainAxisSize.min, children: [Icon(Icons.fingerprint_rounded, size: 17), SizedBox(width: 5), Text('Biyometriyi aç', style: TextStyle(fontSize: 11, fontWeight: FontWeight.w800))]),
                  ),
                ),
              );
            },
...
class _EmailSecuritySetupScreenState extends State<EmailSecuritySetupScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      backgroundColor: AppColors.background,
      appBar: AppBar(title: const Text('E-posta doğrulama'), automaticallyImplyLeading: false, actions: [TextButton(onPressed: _busy ? null : () => FirebaseAuth.instance.signOut(), child: const Text('Çıkış'))]),
      body: SafeArea(child: ListView(padding: const EdgeInsets.all(22), children: [
        ...
        if (_message != null) ...[const SizedBox(height: 14), Text(_message!, textAlign: TextAlign.center, style: const TextStyle(color: Colors.white70))],
      ])),
    );
  }
}
...
class _PhoneSecuritySetupScreenState extends State<PhoneSecuritySetupScreen> {
  ...
  Future<void> _sendCode() async {
    ...
    await FirebaseAuth.instance.verifyPhoneNumber(
      ...
      verificationCompleted: (credential) => _finishVerification(credential, phone),
      verificationFailed: (e) { if (mounted) setState(() { _busy = false; _error = '${e.message ?? 'SMS gönderilemedi.'} (Firebase kodu: ${e.code})'; }); },
      codeSent: (id, _) { if (mounted) setState(() { _verificationId = id; _busy = false; }); },
      codeAutoRetrievalTimeout: (id) { if (mounted) setState(() { _verificationId = id; _busy = false; }); },
    );
  }
  ...
  Future<void> _confirmCode() async {
    ...
    await _finishVerification(PhoneAuthProvider.credential(verificationId: id, smsCode: _code.text.trim()), _phone.text.replaceAll(' ', '').trim());
  }
  ...
  Future<void> _finishVerification(PhoneAuthCredential credential, String phone) async {
    ...
    setState(() { _busy = true; _error = null; });
    try {
      final linked = user.providerData.any((p) => p.providerId == PhoneAuthProvider.PROVIDER_ID);
      if (linked) {
      ...
      if (mounted) setState(() { _busy = false; _error = e.code == 'credential-already-in-use' ? 'Bu telefon numarası başka bir hesapta kullanılıyor.' : '${e.message ?? 'Telefon doğrulanamadı.'} (Firebase kodu: ${e.code})'; });
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      backgroundColor: AppColors.background,
      appBar: AppBar(title: const Text('Hesap güvenliği'), automaticallyImplyLeading: false, actions: [TextButton(onPressed: _busy ? null : () => FirebaseAuth.instance.signOut(), child: const Text('Çıkış'))]),
      body: SafeArea(child: ListView(padding: const EdgeInsets.all(22), children: [
        ...
        const SizedBox(height: 8),
        const Text('İkinci güvenlik katmanı olarak telefon numaranı SMS koduyla doğrula. Numaran yalnızca hesap güvenliği, kötüye kullanım önleme ve hesap kurtarma için kullanılır. Sonraki açılışlarda cihaz destekliyorsa parmak izi veya yüz tanıma kullanabilirsin.', textAlign: TextAlign.center, style: TextStyle(color: Colors.white60, height: 1.45)),
        const SizedBox(height: 26),
        TextField(controller: _phone, enabled: !codeSent && !_busy, keyboardType: TextInputType.phone, decoration: const InputDecoration(labelText: 'Telefon numarası', prefixIcon: Icon(Icons.phone_outlined))),
        if (codeSent) ...[const SizedBox(height: 12), TextField(controller: _code, enabled: !_busy, keyboardType: TextInputType.number, decoration: const InputDecoration(labelText: 'SMS doğrulama kodu', prefixIcon: Icon(Icons.sms_outlined)))],
        if (_error != null) ...[const SizedBox(height: 12), Text(_error!, style: const TextStyle(color: Colors.redAccent))],
        const SizedBox(height: 18),
        FilledButton.icon(onPressed: _busy ? null : (codeSent ? _confirmCode : _sendCode), icon: _busy ? const SizedBox(width: 18, height: 18, child: CircularProgressIndicator(strokeWidth: 2)) : Icon(codeSent ? Icons.verified_outlined : Icons.sms_outlined), label: Text(codeSent ? 'Kodu doğrula' : 'Kod gönder')),
        if (codeSent) TextButton(onPressed: _busy ? null : () => setState(() { _verificationId = null; _code.clear(); _error = null; }), child: const Text('Telefon numarasını değiştir')),
      ])),
    );
  }
}
