  ...
  Widget build(BuildContext context) {
    ...
    if (uid == null) {
      return const Scaffold(
        body: Center(
          child: Text('TBT Yolculuğunu görmek için giriş yapmalısın.'),
        ),
      );
    }
    return Scaffold(
      ...
      body: StreamBuilder<DocumentSnapshot<Map<String, dynamic>>>(
        ...
        builder: (context, snapshot) {
          ...
          final verified =
              user['tbtVerified'] == true || reputation['verified'] == true;
          final ambassador =
              user['tbtAmbassador'] == true || reputation['ambassador'] == true;
          return RefreshIndicator(
            ...
            child: ListView(
              ...
              children: [
                ...
                PublicAchievementBadges(profile: user, showAll: true),
                const SizedBox(height: 18),
                const Text(
                  'Kazanabileceğin unvanlar',
                  style: TextStyle(fontSize: 20, fontWeight: FontWeight.w900),
                ),
                const SizedBox(height: 5),
                const Text(
                  'Kişisel hesabınla başla. Paylaşımların ve katkılarınla unvanlarını kazan.',
                ...
                _RoleCard(
                  ...
                  score: _number(scores['creator']),
                  threshold: 500,
                  state: _map(roles['creator']),
                  requirement: '25 özgün içerik ve en az 60 günlük hesap',
                  earning:
                      'Fotoğraf +5 · Reels +8 · kaliteli içerik bonusu +15',
                ),
                _RoleCard(
                  label: 'TBT Kâşif',
                  icon: Icons.explore_rounded,
                  color: const Color(0xFF42D6C7),
                  score: _number(scores['explorer']),
                  threshold: 400,
                ...
                _RoleCard(
                  ...
                  score: _number(scores['social']),
                  threshold: 350,
                  state: _map(roles['social']),
                  requirement:
                      '5 gerçekleşen etkinlik ve 10 doğrulanmış katılım',
                ...
                _RoleCard(
                  ...
                  score: _number(scores['gourmet']),
                  threshold: 400,
                  state: _map(roles['gourmet']),
                  requirement:
                      '15 farklı mekân katkısı ve 10 fotoğraflı deneyim',
                  earning:
                      'Mekân paylaşımı +7 · yorum +8 · rezervasyon/kupon +5',
                ),
                const SizedBox(height: 18),
        ...
        Expanded(
          child: Column(
            ...
            children: [
              ...
              Text(
                total == 0
                    ? 'İlk paylaşımınla yolculuğuna başla.'
                    : '$total katkı puanı',
                style: const TextStyle(color: Color(0xFFB6C7DA)),
              ),
            ],
          ),
        ),
        if (verified)
          const Icon(Icons.verified_rounded, color: Color(0xFF52D8FF)),
      ],
    ),
  );
}
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            if (active && invited)
              Text(
                'Özel TBT davetiyle aktif · $score gerçek puan',
              ...
              Text(
                active
                    ? 'Hesap aktif · $score puan'
                    : '$score / $threshold puan',
            ...
            ExpansionTile(
              ...
              children: [
                ...
                const Text(
                  'Özel davet ilgili unvanın şartlarını kaldırır; puan kazandırmaz.',
  ...
  Widget build(BuildContext context) => Card(
    ...
    child: ListTile(
      leading: Icon(
        ...
        color: verified ? const Color(0xFF52D8FF) : Colors.white54,
      ),
      title: Text(
        verified ? 'Doğrulanmış hesap' : 'Doğrulanmış hesap ilerlemesi',
        style: const TextStyle(fontWeight: FontWeight.w900),
      ),
      subtitle: Text(
        verified
            ? 'Güven ve katkı şartlarını tamamladın.'
            : '${total.clamp(0, 1000)} / 1000 puan · Telefon, e-posta, hesap yaşı ve temiz kullanım şartları aranır.',
      ),
    ),
  );
}
...
class _AmbassadorCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final completed = const [
      ...
      'social',
      'gourmet',
    ].where((key) => _map(roles[key])['active'] == true).length;
    return Card(
      color: AppColors.surfaceAlt,
      child: ListTile(
        leading: Icon(
          Icons.explore_rounded,
          color: active ? const Color(0xFFFFD166) : Colors.white54,
        ),
        title: const Text(
          'TBT Elçisi',
        ...
        subtitle: Text(
          active
              ? 'Dört hesap türünü ve doğrulamayı tamamladın.'
              : '$completed/4 hesap türü · ${verified ? 'Doğrulama tamam' : 'Doğrulama bekleniyor'}',
        ),
        trailing: active
            ? const Icon(
