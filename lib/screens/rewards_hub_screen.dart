...
class RewardsHubScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (uid == null) {
      return const Scaffold(body: Center(child: Text('Görevleri görmek için giriş yapmalısın.')));
    }
    return Scaffold(
      ...
      body: StreamBuilder<DocumentSnapshot<Map<String, dynamic>>>(
        ...
        builder: (context, snapshot) {
          ...
          final range = (info.$3 - currentFloor).clamp(1, 1000000);
          final progress = info.$3 == xp && info.$1 == _levels.length
              ? 1.0
              : ((xp - currentFloor) / range).clamp(0.0, 1.0);
            ...
            children: [
              Container(
                ...
                child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Row(children: [
                    const CircleAvatar(radius: 25, child: Icon(Icons.workspace_premium_rounded)),
                    const SizedBox(width: 12),
                    Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Text(info.$2, style: const TextStyle(fontSize: 20, fontWeight: FontWeight.w900)),
                      Text('Seviye ${info.$1} • $xp XP', style: const TextStyle(color: Colors.white60)),
                    ])),
                  ]),
                  const SizedBox(height: 16),
                  LinearProgressIndicator(value: progress, minHeight: 8, borderRadius: BorderRadius.circular(10)),
                  const SizedBox(height: 8),
                  Text(info.$1 == _levels.length ? 'En yüksek seviyedesin.' : 'Sonraki seviye için ${info.$3 - xp} XP kaldı.', style: const TextStyle(color: Colors.white60)),
                ]),
              ),
              const SizedBox(height: 18),
              const Text('Bugünün Görevleri', style: TextStyle(fontSize: 19, fontWeight: FontWeight.w900)),
              const SizedBox(height: 8),
              _Mission(title: 'Bir paylaşım yap', subtitle: '+10 XP', done: dailyPost >= 1, progress: dailyPost.clamp(0, 1), target: 1),
              _Mission(title: 'Bir Story paylaş', subtitle: '+5 XP', done: dailyStory >= 1, progress: dailyStory.clamp(0, 1), target: 1),
              _Mission(title: 'Bir etkinliğe katıl', subtitle: '+15 XP', done: dailyJoin >= 1, progress: dailyJoin.clamp(0, 1), target: 1),
              const SizedBox(height: 8),
              ...
              const SizedBox(height: 8),
              _Mission(title: '3 paylaşım yap', subtitle: 'Akışı canlı tut', done: weeklyPost >= 3, progress: weeklyPost.clamp(0, 3), target: 3),
              _Mission(title: 'Bir etkinlik oluştur', subtitle: '+50 XP', done: weeklyEvent >= 1, progress: weeklyEvent.clamp(0, 1), target: 1),
              _Mission(title: '2 etkinlik anısı ekle', subtitle: 'Topluluğa katkı sağla', done: weeklyMemory >= 2, progress: weeklyMemory.clamp(0, 2), target: 2),
              const SizedBox(height: 8),
          ...
          return Container(
            ...
            child: Column(children: List.generate(top.length, (index) {
              ...
              return ListTile(
                ...
                trailing: Text('$xp XP', style: const TextStyle(fontWeight: FontWeight.w900)),
              );
            })),
          );
        },
