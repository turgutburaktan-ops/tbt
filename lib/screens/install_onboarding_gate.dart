...
class _InstallOnboardingGateState extends State<InstallOnboardingGate> {
  ...
  void initState() {
    ...
    }).catchError((Object _) { if (mounted) setState(() => _completed = false); });
  }
  ...
  Future<void> _finish() async {
    ...
    } catch (_) { /* Do not trap users in onboarding when storage is full. */ }
...
class _InstallIntroState extends State<_InstallIntro> {
  ...
  int _page = 0;
  static const _pages = [
    (Icons.explore_rounded, 'introDiscover', 'introDiscoverBody'),
    (Icons.route_rounded, 'introPlan', 'introPlanBody'),
    (Icons.groups_rounded, 'introShare', 'introShareBody'),
  ];
      ...
      child: Column(
        children: [
          Padding(padding: const EdgeInsets.symmetric(horizontal: 24), child: Row(children: [
            DropdownButton<String>(value: AppLocaleService.instance.locale.value.languageCode,
              items: ['tr', 'en', 'de', 'ar'].map((code) => DropdownMenuItem(value: code, child: Text(AppLocaleService.instance.languageName(code)))).toList(),
              onChanged: (code) { if (code != null) AppLocaleService.instance.setLanguage(code); }),
            const Spacer(), TextButton(onPressed: widget.onFinished, child: Text(AppStrings.of(context).text('skip'))),
          ])),
          Expanded(child: PageView.builder(controller: _controller, itemCount: _pages.length, onPageChanged: (value) => setState(() => _page = value), itemBuilder: (_, index) { final item = _pages[index]; return Padding(padding: const EdgeInsets.all(32), child: SingleChildScrollView(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [Container(width: 112, height: 112, decoration: BoxDecoration(gradient: const LinearGradient(colors: [Color(0xFF00BDD6), Color(0xFF285F8C)]), borderRadius: BorderRadius.circular(34)), child: Icon(item.$1, size: 58, color: Colors.white)), const SizedBox(height: 34), Text(AppStrings.of(context).text(item.$2), textAlign: TextAlign.center, style: const TextStyle(fontSize: 30, fontWeight: FontWeight.w900)), const SizedBox(height: 14), Text(AppStrings.of(context).text(item.$3), textAlign: TextAlign.center, style: const TextStyle(color: Colors.white60, fontSize: 16, height: 1.5))]))); })),
