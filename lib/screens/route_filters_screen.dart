...
class RouteFilters {
  ...
  bool matches(TravelPlan p, Set<String> followed) {
    ...
    if (maxKm < 100 && (p.distanceKm <= 0 || p.distanceKm > maxKm))
      return false;
    if (duration > 0 &&
        (p.travelMinutes <= 0 ||
            (duration == 1
...
class RouteFiltersScreen extends StatefulWidget {
  ...
  @override
  State<RouteFiltersScreen> createState() => _RouteFiltersScreenState();
}
...
class _RouteFiltersScreenState extends State<RouteFiltersScreen> {
  ...
  late bool _round = widget.value.roundTrip,
      _following = widget.value.following;
  ...
  Widget build(BuildContext context) => Scaffold(
    ...
    appBar: AppBar(
      title: const Text('Parkur filtreleri'),
      actions: [
        TextButton(
          onPressed:
              () => setState(() {
    ...
    body: ListView(
      ...
      children: [
        DropdownButtonFormField<String>(
          key: ValueKey(_city),
          initialValue: widget.cities.contains(_city) ? _city : '',
          decoration: const InputDecoration(labelText: 'Bölge'),
          items: [
            const DropdownMenuItem(value: '', child: Text('Tüm bölgeler')),
            for (final c in widget.cities)
              DropdownMenuItem(value: c, child: Text(c)),
          ],
          onChanged: (v) => setState(() => _city = v ?? ''),
        ),
        _label('Aktivite türü'),
        Wrap(
          spacing: 8,
          children: [
            for (final m in ['Araç', 'Yürüyüş', 'Bisiklet'])
              FilterChip(
                label: Text(m),
                selected: _mode == m,
                onSelected: (v) => setState(() => _mode = v ? m : ''),
              ),
          ],
        ),
        _label('Mesafe · ${_km == 100 ? 'Tümü' : '0–${_km.round()} km'}'),
        Slider(
          value: _km,
          min: 5,
          max: 100,
        ...
        Wrap(
          ...
          children: [
            for (var i = 1; i <= 3; i++)
              FilterChip(
                label: Text(
                  ['1 saate kadar', '1–3 saat', '3 saatten fazla'][i - 1],
                ),
                selected: _duration == i,
                onSelected: (v) => setState(() => _duration = v ? i : 0),
              ),
          ],
        ),
        _label('Zorluk'),
        ...
        const Text(
          'Zorluk rota sahibinin değerlendirmesidir. Bilgisi olmayan rotalar zorluk filtresinde gösterilmez.',
          style: TextStyle(color: AppColors.textMuted, fontSize: 12),
        ),
        _label('Rota tipi'),
        SwitchListTile(
          contentPadding: EdgeInsets.zero,
          title: const Text('Başlangıca dönüş'),
          value: _round,
          onChanged: (v) => setState(() => _round = v),
        ),
        SwitchListTile(
          contentPadding: EdgeInsets.zero,
          title: const Text('Yalnızca takip ettiklerim'),
          value: _following,
