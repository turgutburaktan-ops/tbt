              ...
              children: [
                ...
                TextField(
                  ...
                  decoration: const InputDecoration(
                    ...
                    hintText: 'Örn. Kurucu işletme / iş ortaklığı',
                  ),
                ),
              ],
            ),
            ...
            child: StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
              ...
              builder: (context, snapshot) {
                ...
                final docs = snapshot.data!.docs.where((doc) {
                  final d = doc.data();
                  final hay =
                      '${d['venueName'] ?? ''} ${d['legalName'] ?? ''} ${d['businessEmail'] ?? ''} ${doc.id}'
                          .toLowerCase();
                  return q.isEmpty || hay.contains(q);
                }).toList();
                if (docs.isEmpty)
                  return const Center(
                    child: Text('Doğrulanmış işletme bulunamadı.'),
                  );
                ...
                return ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    final d = doc.data();
                    final name = (d['venueName'] ?? d['legalName'] ?? 'İşletme')
                        .toString();
                            ...
                            : PopupMenuButton<String>(
                                ...
                                itemBuilder: (_) => [
                                  const PopupMenuItem(
                                    value: 'grant',
                                    child: Text(
                                      'Ücretsiz Pro ver / süreyi uzat',
                                    ),
                                  ),
                                  if (adminActive)
                                    const PopupMenuItem(
                                      value: 'remove',
                                      child: Text('Admin Premium’u kaldır'),
                                    ),
                                ],
                              ),
                      ),
