          ...
          child: StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
            ...
            builder: (context, snapshot) {
              ...
              final docs = snapshot.data!.docs.where((doc) {
                ...
                final d = doc.data();
                final haystack = [d['displayName'], d['username'], d['email'], doc.id]
                    .map((e) => (e ?? '').toString().toLowerCase())
                    .join(' ');
                return haystack.contains(q);
              }).toList();
              if (docs.isEmpty) return const Center(child: Text('Kullanıcı bulunamadı.'));
              return ListView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  final d = doc.data();
                  final name = (d['displayName'] ?? d['username'] ?? 'Kullanıcı').toString();
                  final username = (d['username'] ?? '').toString();
                  final banned = d['banned'] == true;
                  final trust = (d['trustScore'] ?? d['trust'] ?? '').toString();
                  return Card(
                    child: ListTile(
                      ...
                      onTap: () => showModalBottomSheet<void>(
                        ...
                        builder: (_) => ListView(
                          ...
                          children: [
                            ...
                            Text('UID: ${doc.id}', style: const TextStyle(color: Colors.white54)),
                            const SizedBox(height: 16),
                            _InfoRow('Kullanıcı adı', username.isEmpty ? '-' : '@$username'),
                            _InfoRow('E-posta', (d['email'] ?? '-').toString()),
            ...
            builder: (context, snapshot) {
              ...
              return ListView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  final venue = (d['venueName'] ?? d['legalName'] ?? 'İşletme').toString();
                  final early = (d['earlyBusinessAccessUntil'] ?? d['premiumUntil'] ?? '').toString();
                  return Card(
                    child: ListTile(
                      leading: Icon(
                        status == 'verified' ? Icons.verified_rounded : Icons.storefront_outlined,
                        color: status == 'verified' ? AppColors.cyan : Colors.white60,
                      ),
                      title: Text(venue, style: const TextStyle(fontWeight: FontWeight.w900)),
                      subtitle: Text([
                        (d['category'] ?? '').toString(),
                        status,
                      ...
                      onTap: () => showModalBottomSheet<void>(
                        ...
                        builder: (_) => Padding(
                          ...
                          child: Column(
                            ...
                            children: [
                              Text(venue, style: const TextStyle(fontSize: 22, fontWeight: FontWeight.w900)),
                              const SizedBox(height: 14),
                              _InfoRow('Durum', status),
                              _InfoRow('Kategori', (d['category'] ?? '-').toString()),
                              _InfoRow('İşletme e-postası', (d['businessEmail'] ?? '-').toString()),
                              _InfoRow('Yetkili UID', (d['applicantUid'] ?? '-').toString()),
                              _InfoRow('Premium', early.isEmpty ? 'Standart' : 'Erken İşletme Premium'),
                              const SizedBox(height: 12),
                              const Text('Onay/red işlemleri Genel Bakış içindeki işletme inceleme alanından yapılır.', style: TextStyle(color: Colors.white60)),
                            ],
                          ),
                        ),
                      ),
...
class _AdminGrowthScreenState extends State<AdminGrowthScreen> {
  ...
  Future<int> _count(Query<Map<String, dynamic>> q) async => (await q.count().get()).count ?? 0;
  ...
  Future<void> _load() async {
    ...
    final counts = await Future.wait<int>([
      ...
      _count(db.collection('business_claims').where('status', isEqualTo: 'verified')),
      _count(db.collection('business_claims').where('status', isEqualTo: 'pending_review')),
    ]);
    ...
    final cities = cityCounts.entries.toList()..sort((a, b) => b.value.compareTo(a.value));
    if (mounted) {
      setState(() {
        _allowed = true;
        _loading = false;
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      backgroundColor: AppColors.background,
      appBar: AppBar(title: const Text('Büyüme ve Dönüşüm'), actions: [IconButton(onPressed: _load, icon: const Icon(Icons.refresh_rounded))]),
      body: ListView(
        ...
        children: [
          GridView.count(
            ...
            children: _metrics.entries.map((e) => _MetricCard(label: e.key, value: e.value)).toList(),
          ),
          const SizedBox(height: 22),
          const Text('Şehir bazlı kullanıcı yoğunluğu', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w900)),
          const SizedBox(height: 8),
          ..._cities.map((e) => Card(child: ListTile(title: Text(e.key), trailing: Text('${e.value}', style: const TextStyle(fontSize: 20, fontWeight: FontWeight.w900))))),
          const SizedBox(height: 16),
          const Card(
            child: Padding(
              padding: EdgeInsets.all(16),
              child: Text('Bu ekranı ileride kayıt → aktif kullanıcı, etkinlik katılımı, reklam ve işletme → premium dönüşüm oranlarıyla genişletebiliriz. Veri geldikçe oranlar gerçek zamanlı hesaplanacak.'),
            ),
          ),
        ],
      ),
...
class _AdminRolePreviewScreenState extends State<AdminRolePreviewScreen> {
  ...
  static const _roles = <String, ({String label, IconData icon})>{
    ...
    'organizer': (label: 'Etkinlik Düzenleyici', icon: Icons.event_available_outlined),
    'business': (label: 'İşletme', icon: Icons.storefront_outlined),
    'verified': (label: 'Doğrulanmış İşletme', icon: Icons.verified_outlined),
    'premium': (label: 'Premium İşletme', icon: Icons.workspace_premium_outlined),
  };
      ...
      body: Column(children: [
        Container(
          ...
          child: Text(
            ...
            style: const TextStyle(color: Color(0xFF07080C), fontWeight: FontWeight.w900),
          ),
        ),
        SizedBox(
          height: 88,
    ...
    final data = switch (role) {
      'guest' => (
          ...
          items: const [
            ...
            ('Mekanları gez', 'Gezilecek yer, kafe, yeme-içme ve otelleri görebilir.'),
      ...
      'user' => (
          ...
          items: const [
            ...
            ('Etkinlik', 'Katılabilir, ilgileniyorum diyebilir ve etkinlik oluşturabilir.'),
      ...
      'business' => (
          ...
          items: const [
            ...
            ('Şirket bilgileri', 'Yasal unvan, e-posta, telefon ve vergi bilgisi.'),
            ('Yetki kanıtı', 'Belge/fotoğraf yükleyerek manuel doğrulamaya gönderir.'),
      ...
      'verified' => (
          ...
          items: const [
            ...
            ('İstatistikler', 'Görüntülenme ve etkileşim metriklerini takip eder.'),
          ],
        ),
      _ => (
          ...
          items: const [
            ...
            ('Gelişmiş istatistik', 'İçerik ve etkinlik performansını karşılaştırır.'),
            ('Öne çıkarma araçları', 'İleride ücretli olacak görünürlük seçenekleri.'),
            ('Kampanya yönetimi', 'İleride reklam ve promosyon yönetimine bağlanır.'),
          ],
        ),
    };
    return ListView(
      ...
      children: [
        ...
        const SizedBox(height: 16),
        ...data.items.map((item) => _PreviewTile(title: item.$1, subtitle: item.$2)),
        const SizedBox(height: 14),
        const Card(
          child: Padding(
            ...
            child: Row(children: [
              ...
              SizedBox(width: 10),
              Expanded(child: Text('Bu mod yalnız görünüm ve akış kontrolü içindir. Gerçek mesaj, ödeme, doğrulama, ban veya veri oluşturma işlemi yapmaz.')),
            ]),
          ),
        ),
      ],
...
class _PreviewTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Card(
    child: ListTile(
      leading: const Icon(Icons.check_circle_outline_rounded, color: AppColors.cyan),
  ...
  Widget build(BuildContext context) => Container(
    ...
    child: Column(
      ...
      children: [
        ...
        Text(label, maxLines: 2, style: const TextStyle(color: Colors.white60, fontSize: 11.5)),
      ],
    ),
  );
}
