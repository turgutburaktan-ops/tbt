...
class CommunityProfileScreen extends StatelessWidget {
  ...
  Future<void> _createEvent(
    ...
    final admins = (community['adminIds'] as List? ?? const [])
        .map((e) => e.toString())
        .toList();
          ...
          return Padding(
            ...
            child: SingleChildScrollView(
              child: Column(
                ...
                children: [
                  Text(
                    '${(community['name'] ?? 'Topluluk')} adına etkinlik',
                  ...
                  TextField(
                    controller: location,
                    decoration: const InputDecoration(
                      labelText: 'Buluşma adresi / mekân',
                    ),
                  ),
                      ...
                      onPressed: saving
                          ...
                          : () async {
                              ...
                              if (cap < 1) {
                                setState(
                                  () => error =
                                      'Katılımcı kapasitesi en az 1 olmalı.',
                                );
                                return;
                              }
                              if (selectedLocation == null) {
                                setState(
                                  () => error = 'Haritadan kesin konumu seç.',
                                );
                                return;
                              }
                              setState(() {
                              ...
                              try {
                                ...
                                if (context.mounted)
                                  _message(
                                    context,
                                    'Topluluk etkinliği oluşturuldu.',
                                  );
                              } catch (e) {
                                if (sheet.mounted)
                                  setState(() {
    ...
    return Scaffold(
      ...
      body: StreamBuilder<DocumentSnapshot<Map<String, dynamic>>>(
        ...
        builder: (context, snapshot) {
          ...
          final uid = FirebaseAuth.instance.currentUser?.uid;
          final admins = (d['adminIds'] as List? ?? const [])
              .map((e) => e.toString())
              .toList();
          ...
          return ListView(
            ...
            children: [
              Row(
                ...
                children: [
                  const CircleAvatar(
                    ...
                    child: Icon(Icons.groups_2_outlined, size: 34),
                  ),
              ...
              Row(
                children: [
                  StreamBuilder<int>(
                    stream: CommunityService.instance.followerCount(
                      communityId,
                    ),
                    builder: (_, s) => Expanded(
                      child: _Stat(value: '${s.data ?? 0}', label: 'Takipçi'),
                    ),
                  ),
                  const SizedBox(width: 8),
                  Expanded(
                    child: _Stat(value: '${admins.length}', label: 'Yönetici'),
                  ),
                ],
              ),
                ...
                children: [
                  Expanded(
                    child: StreamBuilder<bool>(
                      stream: CommunityService.instance.isFollowing(
                        communityId,
                      ),
                      builder: (_, s) => FilledButton.tonal(
                        onPressed: () async {
                          try {
                            await CommunityService.instance.toggleFollow(
                              communityId,
                            );
                          } catch (e) {
                            if (context.mounted)
                  ...
                  if (isAdmin) ...[
                    ...
                    Expanded(
                      child: FilledButton.icon(
                        onPressed: () => _createEvent(context, d),
              ...
              StreamBuilder<List<SocialEvent>>(
                stream: SocialEventService.instance.watchForCommunity(
                  communityId,
                ),
...
class _EmptyEvents extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
    ...
    child: const Column(
      children: [
        Icon(Icons.event_available_outlined, color: Colors.white38, size: 34),
        SizedBox(height: 8),
        Text(
          'Henüz topluluk etkinliği yok.',
