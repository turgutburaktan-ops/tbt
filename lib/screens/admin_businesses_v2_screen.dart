...
class _AdminBusinessesV2ScreenState extends State<AdminBusinessesV2Screen> {
    ...
    try {
      ...
      for (final doc in venues.docs) {
        ...
        merged[doc.id] = {
          ...data,
          'venueKey': doc.id,
          'venueName': (data['venueName'] ?? data['name'] ?? 'İşletme')
              .toString(),
      ...
      for (final raw in claims) {
        ...
        merged[key] = {
          ...?merged[key],
          ...claim,
          'venueKey': merged[key]?['venueKey'] ?? claim['venueKey'] ?? key,
        };
      }
      final items = merged.values.toList()
        ..sort(
          (a, b) => (a['venueName'] ?? a['legalName'] ?? '')
              .toString()
              .compareTo((b['venueName'] ?? b['legalName'] ?? '').toString()),
        );
      if (mounted)
        setState(() {
          _allowed = true;
    ...
    if (!approve) {
      ...
      final confirmed = await showTbtDialog<bool>(
        ...
        builder: (ctx) => TbtDialog(
          ...
          content: TextField(
            ...
            maxLines: 4,
            maxLength: 500,
            decoration: const InputDecoration(hintText: 'Red nedeni'),
          ),
          actions: [
            TextButton(
      ...
      if (reason.isEmpty) {
        if (mounted)
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Red nedeni yazmalısın.')),
          );
        return;
      }
    } else {
      final confirmed = await showTbtDialog<bool>(
        ...
        builder: (ctx) => TbtDialog(
          ...
          content: const Text(
            'Başvuru sahibine bu işletmenin doğrulanmış yönetim yetkisi verilecek.',
          ),
          actions: [
            TextButton(
              onPressed: () => Navigator.pop(ctx, false),
  ...
  List<Map<String, dynamic>> get _filtered => _items.where((d) {
    ...
    return '${d['venueName'] ?? ''} ${d['legalName'] ?? ''} ${d['businessEmail'] ?? d['ownerEmail'] ?? ''}'
        .toLowerCase()
        .contains(_query);
  }).toList();
  ...
  int _count(String status) => status == 'all'
      ? _items.length
      : _items
            .where((e) => (e['status'] ?? 'registered').toString() == status)
            .length;
  String _status(String s) => s == 'verified'
      ? 'Onaylı'
      : s == 'pending_review'
      ? 'Bekleyen'
      : s == 'rejected'
      ...
      builder: (sheetContext) => DraggableScrollableSheet(
        ...
        builder: (_, controller) => ListView(
          ...
          children: [
            ...
            Text(
              'E-posta: ${data['businessEmail'] ?? data['ownerEmail'] ?? '-'}',
            ),
    ...
    if (action == 'approve') {
      ...
      await Navigator.push(
        ...
        MaterialPageRoute(
          builder: (_) => BusinessPanelPreviewScreen(
            venueName: (data['venueName'] ?? 'İşletme').toString(),
            category: (data['category'] ?? 'cafe').toString(),
          ),
        ),
      );
    } else if (action == 'delete') {
      final name = (data['venueName'] ?? data['legalName'] ?? 'İşletme')
          .toString();
      final confirmed = await showTbtDialog<bool>(
        context: context,
        builder: (context) => TbtDialog(
          title: const Text('İşletmeyi kalıcı sil'),
          content: Text(
      ...
      if (confirmed == true) {
        await AdminConsoleService.instance.deleteVenue(
          collection: 'business_venues',
          id: (data['venueKey'] ?? data['id'] ?? '').toString(),
        );
        await _load();
      }
    }
          ...
          children: [
            ...
            SingleChildScrollView(
              ...
              child: Row(
                children: [
                  _chip('all', 'Tümü'),
                  _chip('registered', 'Kayıtlı'),
                  _chip('pending_review', 'Bekleyen'),
                  _chip('verified', 'Onaylı'),
                  _chip('rejected', 'Reddedilen'),
                ],
              ),
            ),
            const SizedBox(height: 14),
            if (_loading)
              ...
              const Padding(
                padding: EdgeInsets.only(top: 70),
                child: Center(child: Text('Bu filtrede işletme yok.')),
              )
            else
              ..._filtered.map((data) {
                final s = (data['status'] ?? 'registered').toString();
                return Card(
                  child: ListTile(
                    onTap: () => _details(data),
                    leading: Icon(
                      ...
                      color: s == 'verified' ? AppColors.cyan : Colors.white70,
                    ),
                    title: Text(
                      (data['venueName'] ?? data['legalName'] ?? 'İşletme')
                          .toString(),
                      style: const TextStyle(fontWeight: FontWeight.w800),
                    ),
                    subtitle: Text(_status(s)),
