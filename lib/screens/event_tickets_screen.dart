            ...
            return const Center(
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    Icon(Icons.confirmation_number_outlined, size: 62, color: Colors.white30),
                    SizedBox(height: 12),
                    Text('Henüz biletin yok', style: TextStyle(fontWeight: FontWeight.w800)),
                    SizedBox(height: 6),
                    Text('Bir etkinliğe katıldığında biletin burada görünecek.', textAlign: TextAlign.center, style: TextStyle(color: Colors.white54)),
                  ],
                ),
              ),
            );
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              const Text('Girişte bu QR kodu organizatöre göster.', textAlign: TextAlign.center, style: TextStyle(color: Colors.white60)),
            ],
          ),
        ),
      ),
...
class TicketScannerScreen extends StatefulWidget {
  ...
  const TicketScannerScreen({super.key, required this.eventId, required this.eventTitle});
...
class _TicketScannerScreenState extends State<TicketScannerScreen> {
  ...
  Future<void> _handle(BarcodeCapture capture) async {
    ...
    try {
      await EventTicketService.instance.markUsed(qrToken: raw, eventId: widget.eventId);
      if (!mounted) return;
      setState(() => _done = true);
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Bilet doğrulandı ve giriş yapıldı.')));
    } catch (e) {
      if (!mounted) return;
