          ...
          Future<void> toggle() async {
            if (uid == null) {
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(content: Text('Katılmak için giriş yapmalısın.')),
              );
              return;
            }
            try {
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    if (event.city.isNotEmpty) ...[
                      ...
                      _Info(icon: Icons.location_city_outlined, text: event.city),
                    ],
                    if (event.locationLabel.isNotEmpty) ...[
                      const SizedBox(height: 9),
                      _Info(icon: Icons.place_outlined, text: event.locationLabel),
                    ],
                    const SizedBox(height: 9),
                    _Info(
                      icon: Icons.groups_2_outlined,
                      text: '${event.participantCount}/${event.capacity} katılımcı',
                    ),
