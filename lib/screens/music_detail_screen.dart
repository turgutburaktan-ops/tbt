...
class _MusicDetailScreenState extends State<MusicDetailScreen> {
  ...
  Future<void> _loadSaved() async {
    ...
    if (mounted)
      setState(
        () => _saved =
            (prefs.getStringList('story_music_saved') ?? const <String>[])
                .contains(widget.music.trackId),
      );
  }

  Future<void> _toggleSaved() async {
    final prefs = await SharedPreferences.getInstance();
    final ids = (prefs.getStringList('story_music_saved') ?? <String>[])
        .toSet();
  ...
  Future<void> _togglePlay() async {
    ...
    try {
      final track = await FirebaseFirestore.instance
          ...
          .doc(widget.music.trackId)
          .get();
      if (track.data()?['active'] != true)
        throw Exception('Ses kullanılamıyor');
      ...
      await _player.setClip(
        start: Duration(milliseconds: widget.music.startMs),
          ...
          milliseconds: widget.music.startMs + widget.music.clipDurationMs,
        ),
      );
      ...
      if (mounted)
        ScaffoldMessenger.of(context)
            .showSnackBar(const SnackBar(content: Text('Müzik açılamadı.')));
    }
  }
  ...
  Future<void> _report() async {
        ...
        return TbtDialog(
          ...
          content: TextField(
            ...
            decoration: const InputDecoration(
              hintText: 'Nedenini ve hak sahipliği bilgini yaz',
            ),
          ),
          actions: <Widget>[
            TextButton(
    ...
    await FirebaseFirestore.instance.collection('music_takedown_requests').add(
      <String, dynamic>{
    ...
    if (mounted)
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Bildirim incelemeye alındı.')),
      );
  }
      ...
      children: <Widget>[
        ...
        Wrap(
          ...
          children: <Widget>[
            Chip(
              label: Text(
                widget.music.license.isEmpty
                    ? 'Lisans kayıtlı'
                    : widget.music.license,
              ),
            ),
            ...
            StreamBuilder<DocumentSnapshot<Map<String, dynamic>>>(
              ...
              builder: (_, snap) => Chip(
                label: Text(
                  '${(snap.data?.data()?['usageCount'] as num?)?.toInt() ?? 0} kez kullanıldı',
                ),
              ),
            ),
          ],
        ...
        FilledButton.tonalIcon(
          onPressed: () async {
            ...
            if (track.data()?['active'] != true) {
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(content: Text('Bu ses artık kullanılamıyor.')),
              );
              return;
            }
            Navigator.push(
              context,
              MaterialPageRoute(
                builder: (_) => MainCameraScreen(
                  initialMode: CameraShareMode.reels,
                  initialMusic: widget.music,
                ),
              ),
            );
          },
              ...
              children: [
                if (uid.isNotEmpty)
                  TextButton(
                    ...
                    child: const Text('Sanatçının profili'),
                  ),
                if ((t['attributionText'] ?? '').toString().isNotEmpty)
                  Text(t['attributionText'].toString()),
                const Text(
                  'Videolarda seçilen bölüm kesilir ve ses seviyesi değiştirilir.',
          ...
          builder: (_, snap) {
            ...
            if (snap.data!.docs.isEmpty)
              return const Padding(
                ...
                child: Text('İlk videoyu sen oluştur.'),
              );
        ...
        if (widget.music.license == 'CC-BY-4.0')
          TextButton(
            onPressed: () => launchUrl(
        ...
        if (widget.music.sourceUrl.startsWith('https')) ...<Widget>[
          ...
          OutlinedButton.icon(
            onPressed: () => launchUrl(
              Uri.parse(widget.music.sourceUrl),
              mode: LaunchMode.externalApplication,
            ),
