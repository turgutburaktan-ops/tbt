...
class _SpotExploreScreenState extends State<SpotExploreScreen> {
  ...
  Future<void> _refreshRemote() async {
    try {
      final remote = await SpotRepository.instance.discover().timeout(const Duration(seconds: 4));
  ...
  Future<void> _prepareLocation() async {
    try {
      ...
      final position = await Geolocator.getCurrentPosition(locationSettings: const LocationSettings(accuracy: LocationAccuracy.medium)).timeout(const Duration(seconds: 4));
      if (!mounted) return;
      _position = position;
      _applyFilter();
      setState(() {});
  ...
  void _applyFilter() {
    ...
    final next = _all.where((spot) {
      ...
      return '${spot.name} ${spot.city} ${spot.category} ${spot.description} ${spot.tags.join(' ')}'.toLowerCase().contains(key);
    }).toList();
    next.sort((a, b) {
      if (_position != null) {
  ...
  void _toggleRoute(PhotoSpot spot) {
    RouteSelectionService.instance.toggle(RoutePlace(id: _routeId(spot), name: spot.name, category: 'Gezilecek Yerler', latitude: spot.latitude, longitude: spot.longitude));
    setState(() {});
  }

  Future<void> _suggestSpot() async {
    final created = await Navigator.push<bool>(context, MaterialPageRoute(builder: (_) => const SpotSuggestionScreen()));
    if (created == true && mounted) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Önerin admin incelemesine gönderildi.')));
    }
  }
          ...
          child: Padding(
            ...
            child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text(widget.embedded ? 'Gezilecek Yerler' : 'Gezilecek Yerler', style: const TextStyle(fontSize: 22, fontWeight: FontWeight.w900, letterSpacing: -.35)),
                const SizedBox(height: 3),
                Text(_position == null ? 'Türkiye genelindeki gerçek gezi ve çekim noktaları.' : 'Yerler sana en yakından uzağa sıralanıyor.', style: const TextStyle(color: Color(0x75FFFFFF), fontSize: 11.5)),
              ])),
              const SizedBox(width: 8),
              FilledButton.tonalIcon(onPressed: _suggestSpot, icon: const Icon(Icons.add_location_alt_outlined, size: 18), label: const Text('Yer Öner')),
            ]),
          ),
        ),
        SliverToBoxAdapter(
        ...
        if (_loading && _visible.isEmpty)
          ...
          SliverList.builder(
            ...
            itemBuilder: (context, index) {
              ...
              return _SpotVenueCard(spot: spot, selected: selected, distanceLabel: _distanceLabel(spot), onOpen: () => Navigator.push(context, MaterialPageRoute(builder: (_) => SpotDetailScreen(spot: spot))), onToggleRoute: () => _toggleRoute(spot));
            },
          ),
      ...
      child: InkWell(
        ...
        child: Container(
          ...
          child: Row(children: [
            ...
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text(spot.name, maxLines: 2, overflow: TextOverflow.ellipsis, style: const TextStyle(fontWeight: FontWeight.w900, fontSize: 16.5)),
              const SizedBox(height: 4),
              Row(children: [Expanded(child: Text('${spot.city} • ${spot.category}', maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: Colors.white54, fontSize: 11.5))), if (distanceLabel.isNotEmpty) Text(distanceLabel, style: const TextStyle(color: AppColors.cyan, fontWeight: FontWeight.w900, fontSize: 10.5))]),
              const SizedBox(height: 7),
              Row(children: [const Icon(Icons.star_rounded, size: 14, color: AppColors.cyan), const SizedBox(width: 3), Text(spot.rating.toStringAsFixed(1), style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w800)), const SizedBox(width: 10), Expanded(child: Text(spot.bestTime, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: Colors.white38, fontSize: 10.5)))]),
            ])),
            const SizedBox(width: 6),
            IconButton(tooltip: selected ? 'Rotadan çıkar' : 'Rotaya ekle', onPressed: onToggleRoute, style: IconButton.styleFrom(backgroundColor: selected ? AppColors.cyan : AppColors.surfaceStrong, foregroundColor: selected ? const Color(0xFF041311) : Colors.white70), icon: Icon(selected ? Icons.check_rounded : Icons.add_rounded, size: 20)),
          ]),
        ),
      ),
    ),
