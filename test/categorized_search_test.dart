...
void main() {
  testWidgets('explore returns when search is cleared', (tester) async {
    await tester.pumpWidget(
      MaterialApp(
        home: Scaffold(
          body: CategorizedSearch(
            emptyBuilder: (_) => const Text('Keşfet akışı'),
            resultsBuilder: (_, category, query) => Text('Sonuç: $query'),
          ),
        ),
      ),
    );
    expect(find.text('Keşfet akışı'), findsOneWidget);
    await tester.enterText(find.byType(TextField), 'burak');
    await tester.pump(const Duration(milliseconds: 300));
    expect(find.text('Sonuç: burak'), findsOneWidget);
    expect(find.text('Keşfet akışı'), findsNothing);
    await tester.tap(find.byTooltip('Temizle'));
    await tester.pump();
    expect(find.text('Keşfet akışı'), findsOneWidget);
  });

  testWidgets('only selected category loads and typing survives switches', (
    tester,
  ) async {
    ...
    expect(
      find.text('Kişiler içinde aramak için en az 2 karakter yaz.'),
      findsOneWidget,
    );
  });

  testWidgets('rapid input and tab switch discard pending category search', (
    tester,
  ) async {
    final calls = <(SearchCategory, String)>[];
    await tester.pumpWidget(
    ...
    expect(
      calls.every((call) => call == (SearchCategory.places, 'ankara')),
      isTrue,
    );
  ...
  testWidgets('three tabs fit side by side on a narrow screen', (tester) async {
    ...
    await tester.pumpWidget(
      MaterialApp(
        ...
        home: Scaffold(
          body: CategorizedSearch(
            resultsBuilder: (_, __, ___) => const SizedBox(),
          ),
        ),
      ),
    );
