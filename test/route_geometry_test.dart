...
void main() {
  ...
  test('manual geometry survives storage without invented travel time', () {
    ...
    final restored =
        RouteGeometry.decode(
          RouteGeometry.encode(route, 'test', manual: true, roundTrip: true),
        )!;
  ...
  test('origin and round trip affect both waypoints and saved signature', () {
    ...
    expect(
      RouteGeometry.signature([s], 'Yürüyüş', origin: a),
      isNot(
        RouteGeometry.signature([s], 'Yürüyüş', origin: a, roundTrip: true),
      ),
    );
    expect(
      RouteGeometry.signature([s], 'Yürüyüş'),
      isNot(RouteGeometry.signature([s], 'Bisiklet')),
    );
  });
}
